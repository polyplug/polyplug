...
- **D11 — native live-instance counter.** Deferred. The host owns the instance
  ...
  emitted into every native host-caller generator (CI-only validation). If ever needed,
  it would be added via the host/guest contract model, not through any removed extension
  mechanism. Not added pre-freeze without an explicit owner decision. See Decision Point D11.
...
- **D11 — Instance-liveness gating (optional, native-only).** If the runtime should
  *refuse* a native `Reclaim` unload while instances are live — rather than relying
  solely on the `Arc::strong_count` heuristic — a per-bundle live-instance counter
  would be needed. A runtime-side counter is not currently provided: it would either
  duplicate knowledge the host already has, or require auto-increment code emitted into
  every native host-caller generator (CI-only validation). If a concrete need surfaces,
  this would be implemented via the host/guest contract model (not via any removed
  extension mechanism). **Honest caveat:** a live-instance counter *detects* that
  instances remain; it does not make destructor-less reclaim safe, because the
  destructors live in the dylib being closed. The "destroy instances first" host
  contract remains mandatory; a counter would only allow the runtime to enforce it
  rather than assume it.
  **Recommendation:** defer D11 until a concrete need surfaces; if then approved,
  build it in Phase 4 alongside native Reclaim.
