...
## External and internal plugin examples

The checked-in guest examples are **external plugins**: `build_all.sh` generates
guest provider bindings, builds an artifact, and assembles a bundle directory
that a host acquires with `Runtime::load_bundle`. An application that instead
ships an implementation with itself chooses the explicit internal profile for
one bundle:
...
The same command supports `cpp`, `csharp`, `python`, `lua`, and `js-quickjs`. It emits
bundle-identity-namespaced generated guest provider bindings plus generated host
