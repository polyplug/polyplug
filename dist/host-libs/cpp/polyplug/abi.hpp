...
/// Matches ReloadPhaseType in the Rust FFI layer.
enum ReloadPhaseType : uint32_t {
    ReloadPhaseType_Preparing = 0,  ///< Before vtable swap, includes retry count
    ReloadPhaseType_Reloaded  = 1,  ///< After successful vtable swap
    ReloadPhaseType_Failed    = 2   ///< Reload failed, includes reason string
};

/// C-compatible representation of a hot-reload phase.
///
/// This struct is passed to callbacks registered via `polyplug_runtime_on_reload`.
/// The `type` field indicates which variant is active, and the corresponding
...
/// free the memory.
struct ReloadPhase {
    uint32_t   type;         ///< ReloadPhaseType value
    uint64_t   bundle_id;    ///< Bundle ID (valid for all variants)
    StringView bundle_name;  ///< Bundle name (valid for all variants)
    uint32_t   retry_count;  ///< Retry count (valid only for Preparing variant)
    StringView reason;       ///< Failure reason (valid only for Failed variant)
};
...
struct RuntimeConfigC {
    uint32_t hot_reload_max_retries;          ///< Max retry attempts (0 = infinite)
    uint64_t hot_reload_retry_interval_ms;    ///< Interval between retries (ms)
    uint8_t  hot_reload_abort_on_max_retries; ///< Abort when max retries exhausted
};
...
uint32_t polyplug_runtime_on_reload(void (*callback)(ReloadPhase));

/// Set runtime configuration for subsequently created runtimes.
/// Must be called before polyplug_runtime_create.
/// Returns 0 on success, non-zero on error.
