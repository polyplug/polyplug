...
impl CodeGenerator for CppGenerator {
    ...
    fn generate_host(
        ...
        // ── File 5: vtable_factories.hpp ─────────────────────────────────────
        if !ir.host_contracts.is_empty() {
            let vtable_factories_hpp: String = generate_cpp_host_vtable_factories_file(ir);
            files.files.push(GeneratedFile {
                path: std::path::PathBuf::from("host/vtable_factories.hpp"),
                content: vtable_factories_hpp,
                force_regenerate: false,
            });
        }
...
fn generate_cpp_host_vtable_factories_file(ir: &ValidatedIr) -> String {
    let mut out: String = String::new();
    out.push_str("// THIS FILE IS AUTO-GENERATED BY polyplugc. DO NOT EDIT.\n");
    out.push_str("// Re-generate with: polyplugc generate --api api.toml --lang cpp --out <dir>\n");
    out.push_str("#pragma once\n");
    ...
    for contract in &ir.host_contracts {
        generate_cpp_host_vtable_factory(&mut out, contract);
    }
...
fn generate_cpp_host_vtable_factory(out: &mut String, contract: &ResolvedHostContract) {
    let trait_name: String = host_contract_name_to_cpp_trait(&contract.name);
...
mod tests {
    ...
    fn generate_host_with_host_contracts_produces_vtable_factories_file() {
        ...
        let names: Vec<String> = files
            ...
            .collect();
        assert!(
            names.contains(&"host/vtable_factories.hpp".to_owned()),
            "missing vtable_factories.hpp: {names:?}"
        );
    }
    ...
    fn generate_host_without_host_contracts_no_vtable_factories_file() {
        ...
        assert!(
            !names.contains(&"host/vtable_factories.hpp".to_owned()),
            "unexpected vtable_factories.hpp: {names:?}"
        );
    }

    #[test]
    fn generate_cpp_host_vtable_factories_file_produces_file() {
        let ir: ValidatedIr = ValidatedIr {
            types: vec![],
            enums: vec![],
            contracts: vec![],
        ...
        let out: String = generate_cpp_host_vtable_factories_file(&ir);
        assert!(out.contains("AUTO-GENERATED"), "missing header: {out}");
        assert!(
        ...
        assert!(
            out.contains("create_host_logger_vtable"),
            "missing NATIVE factory: {out}"
        );
        assert!(
            out.contains("create_host_logger_vtable_vm"),
    ...
    #[test]
    fn generate_cpp_host_vtable_factory_produces_native_and_vm_factories() {
        let contract = ResolvedHostContract {
            name: "host.logger".to_owned(),
            contract_id: 0x1234_5678_9ABC_DEF0_u64,
            version: Version {
        ...
        assert!(
            out.contains("const HostContractVTable* create_host_logger_vtable"),
            "missing NATIVE factory: {out}"
        );
        assert!(
            out.contains("std::unique_ptr<T> impl"),
