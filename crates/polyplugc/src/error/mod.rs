...
pub(crate) enum CodegenError {
    ...
    #[error("invalid token in value expression `{expr}` for variant `{variant_name}` in enum `{enum_name}`")]
    EnumInvalidValueExpr { enum_name: String, variant_name: String, expr: String },

    #[error("forward reference to variant `{ref_name}` in value expression for `{variant_name}` in enum `{enum_name}`: variant references must be backward-only")]
    EnumForwardRef { enum_name: String, variant_name: String, ref_name: String },

    #[error("chained variant reference: `{variant_name}` references `{ref_name}` which itself references another variant in enum `{enum_name}`: only one level of variant reference is allowed")]
    EnumChainedRef { enum_name: String, variant_name: String, ref_name: String },

    #[error("name `{name}` is used by both a [[type]] and an [[enum]]: names must be unique across both")]
