...
fn write_temp_bundle(name: &str, content: &[u8]) -> (tempfile::TempDir, PathBuf) {
    ...
    fs::write(&path, content).expect("write bundle.lua");
...
fn vtable_is_registered_after_load() {
    ...
    let contract_id: u64 = guest_contract_id("test.loader", 1);
    let handle: Result<GuestContractHandle, RegistryError> = runtime
        .registry()
    ...
    let vtable_ptr: Result<*const GuestContractInterface, RegistryError> =
...
fn vtable_function_count_matches_script() {
    ...
    let contract_id: u64 = guest_contract_id("test.two", 1);
    let handle: Result<GuestContractHandle, RegistryError> = runtime
        .registry()
        .find(GuestContractId::from_u64(contract_id), 0);
...
fn vtable_contract_id_matches_computed_hash() {
    ...
    let expected_cid: u64 = guest_contract_id("test.loader", 1);
    let handle: Result<GuestContractHandle, RegistryError> = runtime
        .registry()
...
fn sequential_loads_both_succeed() {
    ...
    loader
        .load(
            &manifest1,
            &BundleSource::Path(manifest1.path.clone()),
            &runtime,
        )
    ...
    loader
        .load(
            &manifest2,
            &BundleSource::Path(manifest2.path.clone()),
            &runtime,
        )
        .expect("second sequential load must succeed");

    // Both contracts must be visible.
    let cid1: u64 = guest_contract_id("test.loader", 1);
    let cid2: u64 = guest_contract_id("test.two", 1);

    let handle1: Result<GuestContractHandle, RegistryError> =
        runtime.registry().find(GuestContractId::from_u64(cid1), 0);
    assert!(handle1.is_ok(), "test.loader must be registered");

    let handle2: Result<GuestContractHandle, RegistryError> =
...
fn multi_contract_bundle_registers_all_contracts() {
    ...
    let first_cid: u64 = guest_contract_id("test.first", 1);
    let second_cid: u64 = guest_contract_id("test.second", 1);
...
fn concurrent_loaders_do_not_race() {
    ...
    let path_arc: Arc<PathBuf> = Arc::new(path);
    let handles: Vec<thread::JoinHandle<Result<(), LoaderError>>> = (0_u32..4_u32)
        .map(|_| {
            ...
            thread::spawn(move || {
                ...
                let manifest: ManifestData = ManifestData {
                    id: bundle_id("lua_loader_thread_safety"),
        ...
        .collect::<Vec<thread::JoinHandle<Result<(), LoaderError>>>>();
...
fn code_source_loads_and_dispatches_like_path() {
    ...
    let source_text: String =
        fs::read_to_string(&entry).expect("fixture test_plugin.lua must be readable");
    ...
    path_runtime
        .load_bundle_from_source(
            ...
            BundleSource::Path(fixture_dir.clone()),
        )
...
fn bytes_source_with_valid_utf8_loads_and_dispatches() {
    let fixture_dir: PathBuf = lua_fixture_dir();
    let entry: PathBuf = fixture_dir.join("test_plugin.lua");
    let source_bytes: Vec<u8> = fs::read(&entry).expect("fixture test_plugin.lua must be readable");

    let runtime: Arc<Runtime> = make_runtime();
    runtime
        .load_bundle_from_source(
            fixture_manifest(&fixture_dir),
            BundleSource::Bytes(source_bytes),
        )
...
fn bytes_source_with_invalid_utf8_returns_structured_error() {
    ...
    let runtime: Arc<Runtime> = make_runtime();
    let result: Result<(), RuntimeError> = runtime
        .load_bundle_from_source(fixture_manifest(&fixture_dir), BundleSource::Bytes(invalid));
