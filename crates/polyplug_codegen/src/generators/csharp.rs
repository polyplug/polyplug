...
const CS_DOMAIN_TYPES_NAMESPACE: &str = "Polyplug.Generated.DomainTypes";
const CS_GUEST_CONTRACTS_NAMESPACE: &str = "Polyplug.Generated.GuestContracts";

fn cs_partition_namespace(destination: &crate::OutputDestination, fallback: &str) -> String {
    destination
        .import()
...
fn cs_prepend_usings(content: &mut String, namespaces: &[&str]) {
    let mut imports = String::new();
    for namespace in namespaces {
        let using = format!("using {namespace};\n");
        if !content.contains(&using) {
            imports.push_str(&using);
        }
    }
    if !imports.is_empty() {
        let offset = content
            .find(CS_HEADER)
            .map_or(0, |header_start| header_start + CS_HEADER.len());
        content.insert_str(offset, &imports);
    }
}

fn cs_has_domain_types(ir: &ValidatedIr) -> bool {
    !ir.enums.is_empty() || !ir.types.is_empty()
}

fn cs_internal_profile_namespace(bundle_name: &str, bundle_id: u64) -> String {
    let readable = bundle_name
        .chars()
        .map(|character| {
        ...
        .collect::<String>();
    format!("Polyplug.Generated.Internal.Bundle{readable}{bundle_id:016X}")
}

fn wrap_cs_namespace(content: &str, namespace: &str) -> String {
    let system_using = if content.contains("using System;") {
        ""
    } else {
        "using System;\n"
    };
    format!("namespace {namespace} {{\n{system_using}{content}\n}}\n")
}

fn rewrite_cs_file_namespace(content: String, namespace: &str) -> String {
    content.replacen(
...
fn lines_for_csharp_docs(docs: &str) -> Vec<String> {
    super::docs::lines(Some(&xml_escape(docs)))
        .into_iter()
        .map(str::to_owned)
        .collect()
}
    ...
    let cs_enum: CSharpEnum = CSharpEnum {
        ...
        members: e
            ...
            .map(|variant| CSharpEnumMember {
                ...
                docs: variant.docs.as_deref().map(lines_for_csharp_docs),
            })
            .collect(),
        is_flags: e.bitflag,
...
fn cs_assembly_namespace(file: &ResolvedBundleFile) -> Option<String> {
    let raw_path: &str = match file {
        ResolvedBundleFile::Absent => return None,
...
fn generate_cs_internal_host_types_file(ir: &ValidatedIr) -> Result<String, PolyplugcError> {
    let mut out = generate_cs_host_types_file(ir)?;
    for contract in &ir.contracts {
        let struct_name = contract_name_to_cs_class(&contract.name);
        for function in &contract.functions {
            if needs_arg_pack(&function.params) {
                out.push_str(&emit_cs_arg_pack(&struct_name, function)?);
                out.push('\n');
            }
        }
    }
...
fn generate_cs_domain_types_file(
    ir: &ValidatedIr,
    namespace: &str,
) -> Result<String, PolyplugcError> {
...
fn generate_cs_guest_binding_types_file(ir: &ValidatedIr) -> Result<String, PolyplugcError> {
    let mut out = String::new();
    out.push_str(CS_HEADER);
    out.push_str(&csharp_using_block(&[
        &["System.Runtime.InteropServices"],
    ...
    out.push('\n');
    for contract in &ir.contracts {
        let struct_name = pascal_case(&contract.name) + "Contract";
        for function in &contract.functions {
            if needs_arg_pack(&function.params) {
                out.push_str(&emit_cs_arg_pack(&struct_name, function)?);
                out.push('\n');
...
fn generate_cs_host_binding_types_file(ir: &ValidatedIr) -> Result<String, PolyplugcError> {
    let mut out = String::new();
    out.push_str(CS_HEADER);
    out.push_str(&csharp_using_block(&[
        &["System.Runtime.InteropServices"],
    ...
    for contract in &ir.contracts {
        ...
        out.push_str(&format!(
            "    public const ulong {contract_upper}_CONTRACT_ID = 0x{:016X};\n",
...
fn generate_cs_guest_contracts_partition_file(
    ir: &ValidatedIr,
    namespace: &str,
    ...
    let mut out = String::new();
    out.push_str(CS_HEADER);
    let mut namespaces = vec!["Polyplug.Guest", "Polyplug.Abi"];
    if let Some(domain_namespace) = domain_namespace {
        namespaces.push(domain_namespace);
    }
    out.push_str(&csharp_using_block(&[&[], &namespaces]));
    out.push('\n');
    out.push_str(&format!("namespace {namespace};\n\n"));
    for contract in &ir.contracts {
        ...
        for function in &contract.functions {
            out.push_str(&cs_render_interface_method(function)?);
        }
        out.push_str("}\n\n");
    }
    Ok(out)
...
fn generate_cs_internal_host_callers(ir: &ValidatedIr) -> String {
    let mut out = generate_cs_host_callers(ir);
    for contract in &ir.contracts {
        let caller_name = format!("{}Caller", contract_name_to_cs_class(&contract.name));
        let contract_upper = contract.name.to_uppercase().replace(['.', '-'], "_");
        out = out.replacen(
            &format!(
                "public static {caller_name}? Create(Runtime rt) {{\n        var handle = rt.FindGuestContract({}Constants.{contract_upper}_CONTRACT_ID, 0);\n",
                contract_name_to_cs_class(&contract.name)
            ),
            &format!(
                "internal static {caller_name}? CreateFromCommittedHandle(Runtime rt, GuestContractHandle handle) {{\n"
            ),
            1,
        );
        let contract_name = contract_name_to_cs_class(&contract.name);
        for function in &contract.functions {
            if needs_arg_pack(&function.params) {
                let args_name = format!("{}{}Args", contract_name, pascal_case(&function.name));
                out = out.replacen(
                    &format!("ref {args_name} args"),
            ...
            if function.params.len() == 1 {
                ...
                if matches!(&parameter.ty, ResolvedTypeRef::UserDefined(_)) {
                    let ty = cs_type_name(&parameter.ty);
                    out = out.replacen(
                        &format!("ref {ty} {}", parameter.name),
...
/// Generate `guest/Interfaces.cs` for external plugins.
fn generate_cs_guest_interfaces(ir: &ValidatedIr) -> Result<String, PolyplugcError> {
    generate_cs_guest_interfaces_with_internal_plugin_support(ir, false)
}

/// Generate `guest/Interfaces.cs` with the adapter support required by internal plugins.
fn generate_cs_guest_interfaces_with_internal_plugin_support(
    ir: &ValidatedIr,
    include_internal_plugin_support: bool,
) -> Result<String, PolyplugcError> {
    let mut out: String = String::new();
    out.push_str(CS_HEADER);
    out.push_str(&csharp_using_block(&[
            ...
            for contract_impl in &plugin.implements {
                if let Some(contract) = ir.contracts.iter().find(|c| {
                    ...
                    generate_cs_guest_plugin_interface(
                        ...
                        &plugin.name,
                        contract,
                        include_internal_plugin_support,
                    )?;
                }
            }
        }
...
fn emit_cs_guest_instance_machinery(
    ...
    if include_internal_plugin_support {
        out.push_str(&format!(
            "    private sealed class {class_pascal}InternalPluginAdapterState {{ public Func<IntPtr, {iface_name}> Factory = null!; }}\n"
        ));
        out.push_str(&format!(
            "    internal static GuestContractInterface CreateInternalPluginInterface(Func<IntPtr, {iface_name}> factory) {{\n"
        ));
        out.push_str("        ArgumentNullException.ThrowIfNull(factory);\n");
        out.push_str(&format!(
            "        var context = GCHandle.Alloc(new {class_pascal}InternalPluginAdapterState {{ Factory = factory }});\n"
        ));
        ...
        out.push_str("    }\n\n");
        out.push_str(
            "    internal static void ReleaseInternalPluginInterface(GuestContractInterface iface) {\n",
        );
    ...
    let factory_expression: String = if include_internal_plugin_support {
        format!(
            "adapterContext == IntPtr.Zero ? _factory_{lower} : (({class_pascal}InternalPluginAdapterState)GCHandle.FromIntPtr(adapterContext).Target!).Factory"
        )
    } else {
        format!("_factory_{lower}")
    };
    create_body.push_str(&format!(
        "            var factory = {factory_expression};\n"
    ));
...
fn generate_cs_internal_plugin_bundle_file(ir: &ValidatedIr, manifest: String) -> Option<String> {
    ...
    out.push_str("public static class InternalPluginBundleFactory {\n");
    out.push_str("    public static InternalPluginBundle CreateInternalPluginBundle(\n");
    ...
    out.push_str("        );\n");
    out.push_str("        return new InternalPluginBundle(resident.Manifest, resident, resident.RegisterContracts);\n");
    out.push_str("    }\n\n");
    ...
    for (plugin, contract) in &entries {
        ...
        out.push_str(&format!(
            "                {plugin_pascal}Interfaces.CreateInternalPluginInterface({parameter}),\n"
        ));
    }
    out.push_str("            ];\n");
    ...
    for (index, (plugin, _)) in entries.iter().enumerate() {
        let plugin_pascal: String = pascal_case(plugin);
        out.push_str(&format!(
            "            {plugin_pascal}Interfaces.ReleaseInternalPluginInterface(_interfaces[{index}]);\n"
        ));
    }
...
fn generate_cs_host_callers(ir: &ValidatedIr) -> String {
    ...
    for contract in &ir.contracts {
        ...
        // Revalidate through the retained handle after the registry changed.
        // Retain the instance when the resolved interface is unchanged; otherwise
        // its epoch-reclaimed predecessor must be abandoned before replacement.
        ...
        out.push_str(
            "    /// it resolves to null and false is returned (the contract is gone). If an\n",
        );
        out.push_str(
            "    /// unrelated registry change leaves the resolved interface unchanged, the caller\n",
        );
        out.push_str(
            "    /// preserves its instance and only refreshes the cached revision. Otherwise the old\n",
        );
        out.push_str(
        ...
        out.push_str("        if (_host == null) {\n");
        out.push_str("            _interface = null;\n");
        out.push_str("            _instance = default;\n");
        out.push_str("            _disposed = true;\n");
        ...
        out.push_str("        if (_disposed || _interface == null) { return; }\n");
        out.push_str("        ulong revision = LiveRevision();\n");
        out.push_str("        GuestContractInterface* iface = _interface;\n");
        out.push_str("        if (revision != _cachedRevision) {\n");
        out.push_str(
            "            var resolveFn = (delegate* unmanaged[Cdecl]<HostApi*, GuestContractHandle, GuestContractInterface*>)_host->ResolveGuestContract;\n",
        );
        out.push_str("            iface = resolveFn(_host, _handle);\n");
        out.push_str("            if (iface == null) {\n");
        out.push_str("                _interface = null;\n");
        out.push_str("                _instance = default;\n");
        out.push_str("                _cachedRevision = revision;\n");
        out.push_str("                _disposed = true;\n");
        out.push_str("                return;\n");
        out.push_str("            }\n");
        out.push_str("        }\n");
        ...
        out.push_str("        // is destructive. A replaced interface has already reclaimed the old instance.\n");
        out.push_str(
        ...
        out.push_str(&format!(
            "        {create_cast}(_host, iface, null, &newInst);\n"
        ));
        out.push_str("        _interface = iface;\n");
        ...
        out.push_str(
            "            // A revision can belong to an unrelated bundle. Resolve once to\n",
        );
        out.push_str(
            "            // distinguish that case from reload/unload: only the same interface\n",
        );
        out.push_str(
            "            // may destroy this still-live instance; a different or missing one was reclaimed.\n",
        );
        out.push_str("            bool destroyCurrent = LiveRevision() == _cachedRevision;\n");
        out.push_str("            if (!destroyCurrent) {\n");
        out.push_str(
            "                var resolveFn = (delegate* unmanaged[Cdecl]<HostApi*, GuestContractHandle, GuestContractInterface*>)_host->ResolveGuestContract;\n",
        );
        out.push_str("                var resolved = resolveFn(_host, _handle);\n");
        out.push_str("                if (resolved == null) {\n");
        out.push_str("                    _interface = null;\n");
        out.push_str("                    _instance = default;\n");
        out.push_str("                    destroyCurrent = false;\n");
        out.push_str("                } else {\n");
        out.push_str("                    destroyCurrent = resolved == _interface;\n");
        out.push_str("                }\n");
        out.push_str("            }\n");
        out.push_str("            if (destroyCurrent) {\n");
        out.push_str(&format!(
...
fn generate_host_fn_caller(
    ...
    out.push_str("        if (_interface == null || (LiveRevision() != _cachedRevision && !Revalidate())) {\n");
    out.push_str(
...
fn generate_cs_internal_profile_manifest(ir: &ValidatedIr) -> Result<String, PolyplugcError> {
    let bundle = ir
        .bundle
        .as_ref()
        .ok_or_else(|| PolyplugcError::ValidationFailed {
            message: "internal C# generation requires a bundle manifest".to_owned(),
        })?;
    let mut provides: Vec<String> = bundle
        .plugins
        .iter()
...
fn cs_internal_profile_providers(
    ir: &ValidatedIr,
) -> Result<Vec<(&str, &ResolvedContract)>, PolyplugcError> {
    let bundle = ir
        .bundle
        .as_ref()
        .ok_or_else(|| PolyplugcError::ValidationFailed {
    ...
    let mut providers: Vec<(&str, &ResolvedContract)> = Vec::new();
    for plugin in &bundle.plugins {
        for implemented in &plugin.implements {
            let contract = ir
                .contracts
                ...
                .ok_or_else(|| PolyplugcError::ValidationFailed {
                    message: format!(
                        "internal C# provider `{}` references unknown contract `{implemented}`",
                        plugin.name
                    ),
                })?;
            providers.push((plugin.name.as_str(), contract));
        }
    }
    Ok(providers)
}

fn cs_internal_provider_property(plugin: &str, contract: &ResolvedContract) -> String {
    format!(
        "{}{}",
        pascal_case(plugin),
        contract_name_to_cs_class(&contract.name)
...
fn generate_cs_internal_registration_file(
    ir: &ValidatedIr,
    namespace: &str,
    guest_namespace: &str,
    guest_contract_namespace: &str,
    guest_contract_alias: &str,
    host_namespace: &str,
) -> Result<String, PolyplugcError> {
    let providers = cs_internal_profile_providers(ir)?;
    let mut out = String::new();
    out.push_str(CS_HEADER);
    out.push_str(&csharp_using_block(&[
    ...
    if guest_contract_alias != "Guest" {
        out.push_str(&format!(
            "\nusing {guest_contract_alias} = {guest_contract_namespace};\n"
        ));
    }
    out.push_str(&format!(
        "\nusing Guest = {guest_namespace};\nusing Host = {host_namespace};\n\n"
    ));
    out.push_str(&format!("namespace {namespace};\n\n"));
    out.push_str("public sealed class RegistrationInput {\n");
    for (plugin, contract) in &providers {
        let property = cs_internal_provider_property(plugin, contract);
        let interface = contract_name_to_cs_interface(&contract.name);
        out.push_str(&format!(
            "    private Func<nint, {guest_contract_alias}.{interface}>? _{property};\n"
        ));
    }
    out.push_str("    private int _consumed;\n\n");
    out.push_str("    public RegistrationInput(\n");
    for (index, (plugin, contract)) in providers.iter().enumerate() {
        ...
        out.push_str(&format!(
            "        Func<nint, {guest_contract_alias}.{interface}> {property}{separator}\n"
        ));
    }
    out.push_str("    ) {\n");
    for (plugin, contract) in &providers {
    ...
    out.push_str("    }\n\n");
    out.push_str("    internal Providers Consume() {\n");
    out.push_str("        if (Interlocked.Exchange(ref _consumed, 1) != 0) {\n");
    out.push_str(
        "            throw new InvalidOperationException(\"Internal registration input has already been consumed.\");\n",
    );
    out.push_str("        }\n");
    out.push_str("        return new Providers(\n");
    for (index, (plugin, contract)) in providers.iter().enumerate() {
        let property = cs_internal_provider_property(plugin, contract);
        let separator = if index + 1 == providers.len() {
            ""
        ...
        out.push_str(&format!(
            "            Interlocked.Exchange(ref _{property}, null)!{separator}\n"
        ));
    }
    out.push_str("        );\n");
    out.push_str("    }\n");
    out.push_str("}\n\n");
    out.push_str("internal sealed class Providers {\n");
    for (plugin, contract) in &providers {
        let property = cs_internal_provider_property(plugin, contract);
        let interface = contract_name_to_cs_interface(&contract.name);
        out.push_str(&format!(
            "    internal readonly Func<nint, {guest_contract_alias}.{interface}> {property};\n"
        ));
    }
    out.push_str("\n    internal Providers(\n");
    for (index, (plugin, contract)) in providers.iter().enumerate() {
    ...
    for (plugin, contract) in &providers {
        let property = cs_internal_provider_property(plugin, contract);
        out.push_str(&format!("        this.{property} = {property};\n"));
    }
    out.push_str("    }\n");
    out.push_str("}\n\n");
    out.push_str("public sealed class Registration {\n");
    out.push_str("    public ulong BundleId { get; }\n");
    out.push_str("    public GuestContractHandle[] Handles { get; }\n");
    for (plugin, contract) in &providers {
        let property = cs_internal_provider_property(plugin, contract);
        let caller = format!("{}Caller", contract_name_to_cs_class(&contract.name));
        out.push_str(&format!("    public Host.{caller} {property} {{ get; }}\n"));
    }
    out.push_str("\n    internal Registration(ulong bundleId, GuestContractHandle[] handles");
    for (plugin, contract) in &providers {
        let property = cs_internal_provider_property(plugin, contract);
        let caller = format!("{}Caller", contract_name_to_cs_class(&contract.name));
        out.push_str(&format!(", Host.{caller} {property}"));
    }
    out.push_str(") {\n        BundleId = bundleId;\n        Handles = handles;\n");
    for (plugin, contract) in &providers {
        let property = cs_internal_provider_property(plugin, contract);
    ...
    out.push_str("    }\n");
    out.push_str("}\n\n");
    out.push_str("public static class InternalPlugin {\n");
    out.push_str(
        "    public static Registration Register(Runtime runtime, RegistrationInput input) {\n",
    );
    ...
    for (index, (plugin, contract)) in providers.iter().enumerate() {
        ...
        out.push_str(&format!("            providers.{property}{separator}\n"));
    }
    out.push_str("        );\n");
    out.push_str(&format!(
        "        InternalPluginRegistration published = runtime.RegisterInternalPlugin(plugin, {});\n",
        providers.len()
    ));
    out.push_str("        ulong bundleId = published.BundleId;\n");
    out.push_str("        try {\n");
    for (index, (plugin, contract)) in providers.iter().enumerate() {
        let property = cs_internal_provider_property(plugin, contract);
        let caller = format!("{}Caller", contract_name_to_cs_class(&contract.name));
        out.push_str(&format!(
            "            Host.{caller} {property} = Host.{caller}.CreateFromCommittedHandle(runtime, published.Handles[{index}]) ?? throw new InvalidOperationException(\"Generated internal caller creation failed.\");\n"
        ));
    }
    out.push_str("            return new Registration(bundleId, published.Handles");
    for (plugin, contract) in &providers {
        let property = cs_internal_provider_property(plugin, contract);
    ...
    out.push_str(");\n");
    out.push_str("        } catch {\n");
    out.push_str("            runtime.UnloadBundle(bundleId);\n");
    out.push_str("            throw;\n");
    out.push_str("        }\n");
    out.push_str("    }\n");
    out.push_str("}\n");
...
fn add_cs_partition_reference(file: &mut GeneratedFile, partition: crate::OutputPartition) {
    if !file.references.contains(&partition) {
        file.references.push(partition);
    }
}

fn rewrite_cs_split_host_caller_addresses(content: &mut String, ir: &ValidatedIr) {
    *content = content.replace(
        "            nint argsPtr = (nint)(&args);\n",
        "            var argsLocal = args;\n            nint argsPtr = (nint)(&argsLocal);\n",
    );
    for contract in &ir.contracts {
        for function in &contract.functions {
            if function.params.len() != 1
                || !matches!(&function.params[0].ty, ResolvedTypeRef::UserDefined(_))
            {
                continue;
            }
            let name = &function.params[0].name;
            let rewritten = content.replace(
                &format!("            nint argsPtr = (nint)(&{name});\n"),
                &format!(
                    "            var {name}Local = {name};\n            nint argsPtr = (nint)(&{name}Local);\n"
                ),
            );
            *content = rewritten;
        }
...
fn apply_cs_split_layout(
    ...
    let domain_namespace = cs_partition_namespace(&layout.domain_types, CS_DOMAIN_TYPES_NAMESPACE);
    let guest_contract_namespace =
        cs_partition_namespace(&layout.guest_contracts, CS_GUEST_CONTRACTS_NAMESPACE);
    let has_domain_types = cs_has_domain_types(ir);
    ...
    match side {
        Side::Host => {
            for file in &mut files.files {
                ...
                {
                    cs_prepend_usings(&mut file.content, &[&domain_namespace]);
                    add_cs_partition_reference(file, OutputPartition::DomainTypes);
                }
            }
            files.files.push(GeneratedFile {
                path: PathBuf::from("host/DomainTypes.cs"),
                content: generate_cs_domain_types_file(ir, &domain_namespace)?,
        ...
        Side::Guest => {
            ...
            for file in &mut files.files {
                    ...
                    .is_some_and(|extension| extension == "cs")
                {
                    ...
                    if has_domain_types {
                        imports.push(domain_namespace.as_str());
                        add_cs_partition_reference(file, OutputPartition::DomainTypes);
                    }
                    if file.path == Path::new("guest/Interfaces.cs") && !ir.contracts.is_empty() {
                        imports.push(guest_contract_namespace.as_str());
                        add_cs_partition_reference(file, OutputPartition::GuestContracts);
                    }
                    cs_prepend_usings(&mut file.content, &imports);
                }
            }
            files.files.push(GeneratedFile {
                path: PathBuf::from("guest/DomainTypes.cs"),
                content: generate_cs_domain_types_file(ir, &domain_namespace)?,
                force_regenerate: false,
                partition: OutputPartition::DomainTypes,
                references: Vec::new(),
            ...
            files.files.push(GeneratedFile {
                path: PathBuf::from("guest/GuestContracts.cs"),
                content: generate_cs_guest_contracts_partition_file(
                    ir,
                    &guest_contract_namespace,
                    has_domain_types.then_some(domain_namespace.as_str()),
                )?,
                force_regenerate: false,
...
fn apply_cs_internal_split_layout(
    ir: &ValidatedIr,
    layout: &OutputLayout,
    files: &mut GeneratedFiles,
    namespace: &str,
    ...
    let guest_contract_namespace = cs_partition_namespace(
        &layout.guest_contracts,
        &format!("{namespace}.GuestContracts"),
    );
    let has_domain_types = cs_has_domain_types(ir);
    files
        .files
    ...
    for file in &mut files.files {
        ...
        let mut imports = Vec::new();
        if has_domain_types && file.path != Path::new("registration/Registration.cs") {
            imports.push(domain_namespace.as_str());
            add_cs_partition_reference(file, OutputPartition::DomainTypes);
        }
        if (file.path == Path::new("guest/Interfaces.cs")
            || file.path == Path::new("guest/InternalPlugin.cs"))
    ...
    files.files.push(GeneratedFile {
        path: PathBuf::from("domain/DomainTypes.cs"),
        content: generate_cs_domain_types_file(ir, &domain_namespace)?,
        force_regenerate: false,
        partition: OutputPartition::DomainTypes,
        references: Vec::new(),
...
impl CSharpGenerator {
    /// Generate the explicit internal-plugin profile without changing default output.
    pub fn generate_internal_bundle(
        &self,
        ir: &ValidatedIr,
        bundle_name: &str,
        layout: &OutputLayout,
        ...
        self.apply_output_layout(layout)?;
        let bundle = ir
            .bundle
            .as_ref()
            .ok_or_else(|| PolyplugcError::ValidationFailed {
        ...
        let namespace = cs_internal_profile_namespace(bundle_name, bundle.bundle_id);
        let guest_namespace = format!("{namespace}.Guest");
        let host_namespace = format!("{namespace}.Host");
        let guest_contract_namespace = if layout == &OutputLayout::unified() {
            guest_namespace.clone()
        } else {
            cs_partition_namespace(
                &layout.guest_contracts,
                &format!("{namespace}.GuestContracts"),
            )
        };
        let guest_contract_alias = if layout == &OutputLayout::unified() {
            "Guest"
        } else {
            "GuestContracts"
        };
        let mut resident =
            generate_cs_internal_plugin_bundle_file(ir, generate_cs_internal_profile_manifest(ir)?)
                .ok_or_else(|| PolyplugcError::ValidationFailed {
                    message: "internal C# generation requires bundle providers".to_owned(),
                })?;
        if let Some(external_namespace) = cs_assembly_namespace(&bundle.file) {
            resident = resident.replacen(&format!("namespace {external_namespace};\n\n"), "", 1);
        }
        resident = resident.replacen(
            "public static class InternalPluginBundleFactory",
        ...
        files.files.push(GeneratedFile {
            ...
            content: wrap_cs_namespace(&generate_cs_guest_contracts(ir)?, &guest_namespace),
            force_regenerate: false,
            partition: OutputPartition::Bindings,
            references: Vec::new(),
        });
        files.files.push(GeneratedFile {
            ...
            content: wrap_cs_namespace(
                &generate_cs_guest_interfaces_with_internal_plugin_support(ir, true)?,
                &guest_namespace,
            ),
            force_regenerate: false,
            partition: OutputPartition::Bindings,
        ...
        files.files.push(GeneratedFile {
            path: PathBuf::from("guest/InternalPlugin.cs"),
            content: wrap_cs_namespace(&resident, &guest_namespace),
            force_regenerate: false,
            partition: OutputPartition::Bindings,
            references: Vec::new(),
        });
        if !ir.host_contracts.is_empty() {
            files.files.push(GeneratedFile {
                ...
                content: wrap_cs_namespace(
                    &generate_cs_guest_host_contracts_file(ir)?,
                    &guest_namespace,
                ),
                force_regenerate: false,
                partition: OutputPartition::Bindings,
        ...
        if !peer_contracts.is_empty() {
            files.files.push(GeneratedFile {
                ...
                content: rewrite_cs_file_namespace(
                    generate_cs_peer_callers_file(ir, &peer_contracts),
                    &guest_namespace,
                ),
                force_regenerate: false,
                partition: OutputPartition::Bindings,
        ...
        files.files.push(GeneratedFile {
            ...
            content: rewrite_cs_file_namespace(
                generate_cs_internal_host_types_file(ir)?,
                &host_namespace,
            ),
            force_regenerate: false,
            partition: OutputPartition::Bindings,
        ...
        files.files.push(GeneratedFile {
            ...
            content: rewrite_cs_file_namespace(
                generate_cs_internal_host_callers(ir),
                &host_namespace,
            ),
            force_regenerate: false,
            partition: OutputPartition::Bindings,
        ...
        files.files.push(GeneratedFile {
            path: PathBuf::from("registration/Registration.cs"),
            content: generate_cs_internal_registration_file(
                ir,
                &namespace,
                &guest_namespace,
                &guest_contract_namespace,
                guest_contract_alias,
                &host_namespace,
            )?,
            force_regenerate: false,
            partition: OutputPartition::Bindings,
        ...
        apply_cs_internal_split_layout(
            ir,
            layout,
            files,
            &namespace,
...
impl CodeGenerator for CSharpGenerator {
    ...
    fn apply_output_layout(
        ...
        CSharpGenerator::apply_output_layout(self, layout)?;
        apply_cs_split_layout(ir, side, layout, files)
    }
}

impl CSharpGenerator {
    pub(crate) fn apply_output_layout(&self, _layout: &OutputLayout) -> Result<(), PolyplugcError> {
        Ok(())
    }
}
...
mod tests {
    ...
    fn no_peer_callers_without_dependencies() {
        ...
        generator
            .generate_guest(&ir_no_deps, &OutputLayout::unified(), &mut files2)
