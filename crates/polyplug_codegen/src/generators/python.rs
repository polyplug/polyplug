...
fn python_attribute_prefix(indent: &str, node: CustomizableNode, rules: &LanguageRules) -> String {
    render_attributes(Lang::Python, node, rules)
        .into_iter()
        .map(|attribute| format!("{indent}{attribute}\n"))
        .collect()
}

fn emit_python_attributes(
    out: &mut String,
    indent: &str,
    node: CustomizableNode,
    rules: &LanguageRules,
) {
    out.push_str(&python_attribute_prefix(indent, node, rules));
}

fn emit_python_function_metadata(out: &mut String, indent: &str, func: &ResolvedFunction) {
    for param in &func.params {
        emit_python_attributes(out, indent, CustomizableNode::Param, &param.langs);
    }
    emit_python_attributes(out, indent, CustomizableNode::Return, &func.return_langs);
}

fn emit_python_root_attributes(out: &mut String, ir: &ValidatedIr) {
    emit_python_attributes(out, "", CustomizableNode::Api, &ir.langs);
}
...
fn generate_python_domain_types_file(ir: &ValidatedIr) -> Result<String, PolyplugcError> {
    ...
    emit_python_root_attributes(&mut out, ir);
...
fn generate_python_guest_protocol(
    ...
    emit_python_attributes(out, "", CustomizableNode::GuestContract, &contract.langs);
...
fn apply_python_output_layout(
    ...
    for file in &mut files.files {
        if matches!(
            ...
            if !omit_guest_contracts {
                file.content = format!(
                    "{PY_HEADER}{}from {guest_runtime_import} import *\n",
                    python_attribute_prefix("", CustomizableNode::Api, &ir.langs)
                );
                file.references.push(OutputPartition::GuestContracts);
            }
...
fn generate_host_callers_file(ir: &ValidatedIr) -> String {
    ...
    out.push_str("POLYPLUG_ABI_VERSION: int = 2\n");
...
fn generate_python_user_type(out: &mut String, ty: &ResolvedType, enums: &[EnumDef]) {
    emit_python_attributes(out, "", CustomizableNode::Type, &ty.langs);
    ...
    for field in &ty.fields {
        ...
        emit_python_attributes(out, "        ", CustomizableNode::Field, &field.langs);
...
fn generate_host_caller_method(
    ...
    emit_python_function_metadata(out, "    ", func);
    emit_python_attributes(out, "    ", CustomizableNode::Function, &func.langs);
    out.push_str(&format!(
        "    def {}(self{}) -> {}:\n",
...
fn generate_guest_trait_method(
    ...
    let decorator: String =
        python_attribute_prefix("    ", CustomizableNode::Function, &func.langs);
    let backend: PythonBackend = PythonBackend::default();
    let mut indent_level: i32 = 1;
    let rendered: String = backend
        .render_function(
            &method,
            (!decorator.is_empty()).then_some(decorator.as_str()),
            None::<&str>,
            None,
            &mut indent_level,
        )
...
fn generate_python_host_contract_abc(out: &mut String, contract: &ResolvedHostContract) {
    ...
    emit_python_attributes(out, "", CustomizableNode::HostContract, &contract.langs);
...
fn generate_host_contracts_stub(ir: &ValidatedIr) -> String {
    ...
    for contract in &ir.host_contracts {
        ...
        for func in &contract.functions {
            ...
            emit_python_function_metadata(&mut out, "    ", func);
            emit_python_attributes(&mut out, "    ", CustomizableNode::Function, &func.langs);
            if let Some(docs) = python_function_docs(func) {
                out.push_str(&format!(
                    "    @abstractmethod\n    def {}(self{}{}) -> {}:\n",
                    func.name,
...
fn generate_python_guest_host_contract_method(
    ...
    emit_python_function_metadata(dst, "    ", func);
    let decorator: String =
        python_attribute_prefix("    ", CustomizableNode::Function, &func.langs);
...
fn generate_python_enum(out: &mut String, e: &EnumDef) -> Result<(), PolyplugcError> {
    ...
    for variant in &e.variants {
        let member: String = format!("    {} = ", to_upper_snake_case(&variant.name));
        let attributes: String =
            python_attribute_prefix("    ", CustomizableNode::EnumVariant, &variant.langs);
        if !attributes.is_empty() {
            rendered = rendered.replacen(&member, &format!("{attributes}{member}"), 1);
        }
    }
    if !docs.is_empty() {
    ...
    emit_python_attributes(out, "", CustomizableNode::Enum, &e.langs);
    out.push_str(&rendered);
    Ok(())
}
