...
fn generate_host_caller_class_deno(out: &mut String, contract: &ResolvedContract) {
    ...
    for func in &contract.functions {
        ...
        out.push_str("        const fn = fnPtr as unknown as (args: any, out: any) => { code: number; _pad: number; message_ptr: bigint; message_len: number };\n");
        out.push_str("        const outVal = { value: 0n };\n");
        out.push_str("        const result = fn(argsPtr, outVal);\n");
        out.push_str("        if (result.code !== 0) throw new Error('call failed');\n");
