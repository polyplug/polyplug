...
#![allow(dead_code)]\n#![allow(non_snake_case)]\n#![allow(non_camel_case_types)]\n#![allow(clippy::all)]\n#![allow(clippy::pedantic)]\n#![allow(clippy::nursery)]\n\n";
        ...
        if internal_mode == Some(InternalBindingMode::Profile) {
            ...
            generate_internal_profile_domain_file(&mut domain_out, ir)?;
            domain_out.push_str(
                "pub use super::interfaces::{InternalProviderFactory, InternalProviders};\n",
            );
            files.files.push(GeneratedFile {
        ...
        generate_guest_interfaces_file(&mut interfaces_out, ir, internal_mode)?;
        ...
        let guest_mod_content: String = generate_guest_mod_rs(ir, internal_mode);
        files.files.push(GeneratedFile {
...
fn generate_guest_interfaces_file(
    ...
    if let Some(bundle) = &ir.bundle {
            ...
            for contract_impl in &plugin.implements {
                if let Some(contract) = ir.contracts.iter().find(|contract| {
                    ...
                    generate_guest_plugin_interface(
                        ...
                        &plugin_symbol,
                        contract,
                        internal_mode == Some(InternalBindingMode::Profile)
        ...
        for contract in &ir.contracts {
            generate_guest_contract_interface(out, contract, true, internal_mode)?;
        }
    }
    Ok(())
}

fn generate_internal_profile_domain_file(
    out: &mut String,
    ir: &ValidatedIr,
) -> Result<(), PolyplugcError> {
    ...
    for enum_def in &ir.enums {
        out.push_str(&format!("pub use super::types::{};\n", enum_def.name));
    }
...
fn generate_internal_profile_adapters(
    ...
    for provider in guest_providers(ir) {
        let adapter: String = profile_adapter_name(&internal_provider_symbol(&provider));
        ...
        out.push_str(&format!(
            "struct {adapter} {{\n    implementation: Box<dyn super::domain::{domain_trait}>,\n{arena_field}    host: HostContext,\n}}\n\n"
        ));
        out.push_str(&format!(
            "impl {adapter} {{\n    fn new(implementation: Box<dyn super::domain::{domain_trait}>, host: HostContext) -> Self {{\n        Self {{ implementation, {arena_initializer}host }}\n    }}\n}}\n\n"
        ));
            ...
            let mapped = fields
                ...
                .map(|field| {
                    format!(
                        "{}: {}",
                        field.name,
                        domain_from_abi_expr(&field.ty, &format!("{value}.{}", field.name), ir)
                    )
                })
                .collect::<Vec<String>>()
                .join(", ");
...
fn generate_internal_factory_registry(out: &mut String, ir: &ValidatedIr) {
    ...
    for provider in &providers {
        ...
        out.push_str(&format!(
            "    pub {lower}: InternalProviderFactory<dyn super::domain::{trait_name}>,\n"
        ));
    }
    out.push_str("}\n\n");
    for provider in &providers {
        ...
        let domain_trait: String = contract_name_to_struct(&provider.contract.name);
        out.push_str(&format!(
            "unsafe fn internal_provider_factory_{lower}(adapter_context: *mut c_void, _host_ctx: HostContext) -> Option<Box<dyn super::domain::{domain_trait}>> {{\n"
        ));
...
fn generate_guest_contract_interface(
    ...
    emit_guest_instance_machinery(
        ...
        GuestInstanceMachinery {
            ...
            profile_adapter: (internal_mode == Some(InternalBindingMode::Profile))
                .then(|| profile_adapter_name(&lower)),
        },
    )?;
...
fn generate_guest_plugin_interface(
    ...
    emit_guest_instance_machinery(
        ...
        GuestInstanceMachinery {
            ...
            profile_adapter: (internal_mode == Some(InternalBindingMode::Profile))
                .then(|| profile_adapter_name(&plugin_lower)),
        },
    )?;
...
fn generate_internal_profile_guest_init_file(
    ...
    out.push_str(&rust_use_block(&[
        ...
        &["super::domain::InternalProviders"],
    ]));
    out.push('\n');
    out.push_str(&format!(
...
fn generate_guest_mod_rs(ir: &ValidatedIr, internal_mode: Option<InternalBindingMode>) -> String {
    let mut out: String = String::new();
