...
impl RustGenerator {
    pub(crate) fn generate_internal_bundle(
        ...
        if *layout != OutputLayout::unified()
            && matches!(&layout.domain_types, OutputDestination::Inline)
        {
            let Some(host_mod) = host_files
                .files
                .iter_mut()
                .find(|file| file.path == Path::new("host/mod.rs"))
            else {
                return Err(PolyplugcError::ValidationFailed {
                    message: "generated host module is missing".to_owned(),
                });
            };
            host_mod.content = host_mod
                .content
                .replace("pub mod domain;\n", "pub use super::guest::domain;\n");
            host_mod.references.push(OutputPartition::DomainTypes);
        }
        host_files
            .files
            .retain(|file| file.path != Path::new("mod.rs"));
        files.files.extend(host_files.files);
        self.generate_guest_with_mode(
            ir,
            files,
    ...
    pub(crate) fn generate_partitioned_declarations(
        &self,
        ir: &ValidatedIr,
        side: Side,
        files: &mut GeneratedFiles,
        ...
        let domain_path = match side {
            ...
            Side::Guest => PathBuf::from("guest/domain.rs"),
        };
        files.files.push(GeneratedFile {
            path: domain_path,
            content: domain_out,
        ...
        if side == Side::Guest {
            ...
            files.files.push(GeneratedFile {
                ...
                force_regenerate: false,
                partition: OutputPartition::GuestContracts,
                references: vec![crate::OutputPartition::DomainTypes],
            });
        }
        Ok(())
    }
    ...
    fn generate_guest_with_mode(
        ...
        let internal_mode: Option<InternalBindingMode> = match mode {
            GuestRustOutputMode::Disk => None,
            GuestRustOutputMode::InternalProfile { .. } => Some(InternalBindingMode::Profile),
        };
        let internal_bundle_name: Option<&str> = match mode {
            GuestRustOutputMode::Disk => None,
            GuestRustOutputMode::InternalProfile { bundle_name } => Some(bundle_name),
        };
        let domain_api = internal_mode == Some(InternalBindingMode::Profile)
            || *files.layout() != OutputLayout::unified();
...
#![allow(dead_code)]\n#![allow(non_snake_case)]\n#![allow(non_camel_case_types)]\n#![allow(clippy::all)]\n#![allow(clippy::pedantic)]\n#![allow(clippy::nursery)]\n\n";
        ...
        generate_guest_init_file(&mut init_out, ir, internal_bundle_name, internal_mode)?;
        ...
        if !peer_contracts.is_empty() {
            let peer_callers_out: String = generate_peer_callers_file(
                ir,
                &peer_contracts,
                (*files.layout() != OutputLayout::unified()).then_some(ir),
            );
            files.files.push(GeneratedFile {
        ...
        let guest_mod_content: String = generate_guest_mod_rs(ir, internal_mode, domain_api);
        files.files.push(GeneratedFile {
        ...
        if internal_bundle_name.is_none() && ir.bundle.is_some() {
...
impl CodeGenerator for RustGenerator {
    fn generate_host(
        ...
        let domain_api = !matches!(
            layout.destination(OutputPartition::DomainTypes),
            OutputDestination::Inline | OutputDestination::Omit
        );
        let header: &str = "// THIS FILE IS AUTO-GENERATED BY polyplugc. DO NOT EDIT.\n\
    // Re-generate with: polyplugc generate --api api.toml --lang rust --out <dir>\n\
    #![allow(unused_imports)]\n\
    #![allow(dead_code)]\n#![allow(non_snake_case)]\n#![allow(non_camel_case_types)]\n#![allow(clippy::all)]\n#![allow(clippy::pedantic)]\n#![allow(clippy::nursery)]\n\n";
        ...
        for contract in &ir.contracts {
            generate_host_contract_caller(&mut callers_out, contract, domain_api.then_some(ir))?;
        }
    ...
    fn apply_output_layout(
        ...
        if *layout != OutputLayout::unified() {
            self.generate_partitioned_declarations(ir, side, files)?;
        }
        apply_rust_layout_imports(files, layout);
        Ok(())
    }
}

pub(crate) fn apply_rust_layout_imports(output: &mut GenerateOutput, layout: &OutputLayout) {
    let domain_import = layout
        .domain_types
        .import()
        .map(|import| import.as_str().to_owned());
    let guest_contract_import = layout
        .guest_contracts
        .import()
        .map(|import| import.as_str().to_owned());
    for file in &mut output.files {
        if file.content.contains("super::domain") {
            file.references.push(OutputPartition::DomainTypes);
            if let Some(import) = &domain_import {
                file.content = file.content.replace("super::domain", import);
            }
        }
        if file.content.contains("super::guest_contracts") {
            file.references.push(OutputPartition::GuestContracts);
            if let Some(import) = &guest_contract_import {
                file.content = file.content.replace("super::guest_contracts", import);
            }
        }
        if file.path.ends_with(Path::new("guest/mod.rs")) {
            if file.content.contains("pub mod domain;\n") {
                match (&layout.domain_types, &domain_import) {
                    (OutputDestination::Inline, _) => {
                        file.references.push(OutputPartition::DomainTypes);
                    }
                    (_, Some(import)) => {
                        file.content = file.content.replace(
                            "pub mod domain;\n",
                            &format!("pub use {import} as domain;\n"),
                        );
                    }
                    (OutputDestination::Omit, None) => {
                        file.content = file.content.replace("pub mod domain;\n", "");
                    }
                    _ => unreachable!("all destinations are covered"),
                }
            }
            if file.content.contains("pub mod guest_contracts;\n") {
                match (&layout.guest_contracts, &guest_contract_import) {
                    (OutputDestination::Inline, _) => {
                        file.references.push(OutputPartition::GuestContracts);
                    }
                    (_, Some(import)) => {
                        file.content = file.content.replace(
                            ...
                            &format!("pub use {import} as guest_contracts;\n"),
                        );
                    }
                    (OutputDestination::Omit, None) => {
                        file.content = file.content.replace("pub mod guest_contracts;\n", "");
                    }
                    _ => unreachable!("all destinations are covered"),
                }
            }
        }
        if file.path.ends_with(Path::new("host/mod.rs"))
            && file.content.contains("pub mod domain;\n")
        {
            match (&layout.domain_types, &domain_import) {
                (OutputDestination::Inline, _) => {
                    file.references.push(OutputPartition::DomainTypes);
...
fn internal_provider_symbol(provider: &GuestProvider<'_>) -> String {
    format!(
        "{}_{}",
        provider
            .plugin_name
            .unwrap_or(&provider.contract.name)
            .to_lowercase()
            .replace('.', "_"),
        provider.contract.name.to_lowercase().replace('.', "_")
    )
}

fn internal_provider_upper(provider: &GuestProvider<'_>) -> String {
    internal_provider_symbol(provider).to_uppercase()
}
...
fn generate_guest_interfaces_file(
    out: &mut String,
    ir: &ValidatedIr,
    internal_mode: Option<InternalBindingMode>,
    domain_api: bool,
) -> Result<(), PolyplugcError> {
    let mut standard_imports: Vec<&str> = vec!["core::ffi::c_void", "core::ptr"];
    if internal_mode.is_some() {
        standard_imports.push("std::sync::Arc");
    }
    if internal_mode == Some(InternalBindingMode::Profile) {
        standard_imports.push("std::sync::Mutex");
        standard_imports.push("std::collections::HashMap");
        standard_imports.push("std::thread::ThreadId");
    }
    let mut abi_imports: Vec<&str> = vec![
        "polyplug_abi::AbiError",
        "polyplug_abi::AbiErrorCode",
        "polyplug_abi::GuestContractInterface",
        "polyplug_abi::GuestContractInstance",
    ...
    if internal_mode == Some(InternalBindingMode::Profile) {
        abi_imports.push("polyplug_abi::Buffer");
        abi_imports.push("polyplug_guest::ReturnArena");
    }
    ...
    if internal_mode.is_some() {
        generate_internal_factory_registry(out, ir);
    }
    if domain_api {
        generate_internal_profile_adapters(out, ir)?;
    }
    ...
    if let Some(bundle) = &ir.bundle {
        ...
        for plugin in &bundle.plugins {
            for contract_impl in &plugin.implements {
                if let Some(contract) = ir.contracts.iter().find(|contract| {
                    let contract_full: String = format!(
                        "{}@{}.{}",
                        contract.name, contract.version.major, contract.version.minor
                    );
                    &contract_full == contract_impl
                }) {
                    let plugin_symbol: String = match internal_mode {
                        Some(InternalBindingMode::Profile) => format!(
                            "{}_{}",
                    ...
                    generate_guest_plugin_interface(
                        out,
                        GuestPluginInterfaceConfig {
                            plugin_name: &plugin.name,
                            plugin_symbol: &plugin_symbol,
                            contract,
                            is_native: internal_mode == Some(InternalBindingMode::Profile)
                                || is_native_runtime(&bundle.loader),
                            emitted_consts: &mut emitted_consts,
                            internal_mode,
                            domain_api,
                        },
                    )?;
        ...
        for contract in &ir.contracts {
            generate_guest_contract_interface(out, contract, true, internal_mode, domain_api)?;
        }
    }
    Ok(())
}

fn generate_internal_profile_domain_types_file(
    out: &mut String,
    ir: &ValidatedIr,
) -> Result<(), PolyplugcError> {
...
fn domain_type_name(ty: &ResolvedTypeRef, types: &[ResolvedType], enums: &[EnumDef]) -> String {
    match ty {
        ResolvedTypeRef::Primitive(primitive) => primitive.rust_name().to_owned(),
        ...
        ResolvedTypeRef::UserDefined(name) => {
            if let Some(element) = array_element_name(name) {
                return format!("Vec<{}>", domain_named_type(element, types, enums));
            }
            name.clone()
        }
    }
...
fn domain_named_type(name: &str, types: &[ResolvedType], enums: &[EnumDef]) -> String {
    if let Some(primitive) = PrimitiveType::parse(name) {
        return primitive.rust_name().to_owned();
    }
    if name == "StringView" {
...
fn generate_internal_profile_adapters(
    out: &mut String,
    ir: &ValidatedIr,
) -> Result<(), PolyplugcError> {
    let mut emitted: HashSet<String> = HashSet::new();
    for provider in guest_providers(ir) {
        let adapter_symbol = provider.plugin_name.map_or_else(
            || provider.contract.name.to_lowercase().replace('.', "_"),
            |_| internal_provider_symbol(&provider),
        );
        let adapter: String = profile_adapter_name(&adapter_symbol);
        if !emitted.insert(adapter.clone()) {
            continue;
        }
        let abi_trait: String = contract_name_to_guest_trait(&provider.contract.name);
        let domain_trait: String = contract_name_to_struct(&provider.contract.name);
        let adapter_needs_arenas = provider.contract.functions.iter().any(|function| {
            function
                .returns
                .as_ref()
                .is_some_and(|return_type| return_needs_arena(return_type, ir))
        });
        let arena_field = if adapter_needs_arenas {
            "    // Per-thread arenas are adapter-owned and drop with this instance.\n    return_arenas: Mutex<HashMap<ThreadId, ReturnArena>>,\n"
        } else {
            ""
        };
        let arena_initializer = if adapter_needs_arenas {
            "return_arenas: Mutex::new(HashMap::new()), "
        } else {
            ""
        };
        out.push_str(&format!(
            "struct {adapter} {{\n    implementation: Box<dyn super::guest_contracts::{domain_trait}>,\n{arena_field}    host: HostContext,\n}}\n\n"
        ));
        out.push_str(&format!(
            "impl {adapter} {{\n    fn new(implementation: Box<dyn super::guest_contracts::{domain_trait}>, host: HostContext) -> Self {{\n        Self {{ implementation, {arena_initializer}host }}\n    }}\n}}\n\n"
        ));
        out.push_str(&format!("impl {abi_trait} for {adapter} {{\n"));
        for function in &provider.contract.functions {
            let params: String = function
                .params
                .iter()
                .map(|param| {
                    let abi_type = rust_type_name(&param.ty);
                    if function.params.len() == 1
                        && matches!(param.ty, ResolvedTypeRef::UserDefined(_))
                    {
                        format!("{}: &{abi_type}", param.name)
                    } else {
                        format!("{}: {abi_type}", param.name)
                    }
                })
                .collect::<Vec<String>>()
                .join(", ");
            let abi_return: String = function
                .returns
                .as_ref()
                .map(rust_type_name)
                .unwrap_or_else(|| "()".to_owned());
            out.push_str(&format!(
                "    fn {}(&self{}{}) -> Result<{abi_return}, GuestError> {{\n",
                function.name,
                if params.is_empty() { "" } else { ", " },
                params
            ));
            for param in &function.params {
                let source = if function.params.len() == 1
                    && matches!(param.ty, ResolvedTypeRef::UserDefined(_))
                {
                    param.name.clone()
                } else {
                ...
                out.push_str(&format!(
                    "        let {} = {};\n",
                    param.name,
                    domain_from_abi_expr(&param.ty, &source, ir)
                ));
            }
            ...
            if let Some(return_type) = &function.returns {
                out.push_str(&format!(
                    "        let value = self.implementation.{}({arguments})?;\n",
                    function.name
                ));
                if return_needs_arena(return_type, ir) {
                    out.push_str("        let mut arenas = self.return_arenas.lock().map_err(|_| GuestError { code: AbiErrorCode::Generic, message: \"return arena map poisoned\".to_owned() })?;\n        let arena = arenas.entry(std::thread::current().id()).or_insert_with(|| ReturnArena::new(self.host, 4096));\n        arena.reset();\n");
                }
                out.push_str(&format!(
                    "        {}\n",
                    abi_from_domain_expr(return_type, "value", ir)
                ));
            } else {
                out.push_str(&format!(
...
fn domain_from_abi_expr(ty: &ResolvedTypeRef, value: &str, ir: &ValidatedIr) -> String {
    match ty {
        ...
        ResolvedTypeRef::AbiType(AbiBuiltin::StringView) => format!(
            "unsafe {{ ({value}).try_as_str() }}.map_err(|error| GuestError {{ code: AbiErrorCode::Generic, message: error.to_string() }})?.to_owned()"
        ),
        ResolvedTypeRef::AbiType(AbiBuiltin::Buffer) => {
            format!("unsafe {{ ({value}).as_slice() }}.to_vec()")
        }
        ResolvedTypeRef::UserDefined(name) if ir.enums.iter().any(|item| item.name == *name) => {
            let Some(enum_def) = ir.enums.iter().find(|item| item.name == *name) else {
                unreachable!("validated enum is present");
            };
            if enum_def.bitflag {
                return format!("*({value})");
            }
        ...
        ResolvedTypeRef::UserDefined(name) if let Some(element) = array_element_name(name) => {
            ...
            format!(
                "unsafe {{ ({value}).as_slice() }}.iter().map(|item| -> Result<_, GuestError> {{ Ok({}) }}).collect::<Result<Vec<_>, GuestError>>()?",
                domain_from_abi_expr(&element_ty, "item", ir)
            )
        }
        ResolvedTypeRef::UserDefined(name) => {
            ...
            let mapped = fields
                ...
                .collect::<Vec<String>>()
                .join(", ");
            format!("super::domain::{name} {{ {mapped} }}")
        }
    }
}

fn abi_from_domain_expr(ty: &ResolvedTypeRef, value: &str, ir: &ValidatedIr) -> String {
    format!("Ok({})", abi_domain_value_expr(ty, value, ir))
}

fn abi_domain_value_expr(ty: &ResolvedTypeRef, value: &str, ir: &ValidatedIr) -> String {
    match ty {
        ResolvedTypeRef::Primitive(_) | ResolvedTypeRef::AbiType(AbiBuiltin::Ptr) => {
            value.to_owned()
        }
        ResolvedTypeRef::AbiType(AbiBuiltin::Void) => "()".to_owned(),
        ResolvedTypeRef::AbiType(AbiBuiltin::StringView) => format!("arena.alloc_str(&{value})"),
        ResolvedTypeRef::AbiType(AbiBuiltin::Buffer) => {
            format!("self.host.alloc_buffer(&{value})?")
        }
        ResolvedTypeRef::UserDefined(name) if ir.enums.iter().any(|item| item.name == *name) => {
            let Some(enum_def) = ir.enums.iter().find(|item| item.name == *name) else {
                unreachable!("validated enum is present");
        ...
        ResolvedTypeRef::UserDefined(name) if let Some(element) = array_element_name(name) => {
            let element_ty = resolved_type_ref_for_name(element);
            let abi_element = rust_type_name(&element_ty);
            format!(
                "{{ let values: Vec<{abi_element}> = {value}.into_iter().map(|item| Ok({})).collect::<Result<Vec<_>, GuestError>>()?; let (items, len) = unsafe {{ arena.alloc_abi_array(&values) }}; super::types::{name} {{ items, len }} }}",
                abi_domain_value_expr(&element_ty, "item", ir)
            )
        }
        ResolvedTypeRef::UserDefined(name) => {
            ...
            let mapped = fields
                ...
                .map(|field| {
                    format!(
                        "{}: {}",
                        field.name,
                        abi_domain_value_expr(&field.ty, &format!("{value}.{}", field.name), ir,)
                    )
                })
                .collect::<Vec<String>>()
                .join(", ");
            format!("super::types::{name} {{ {mapped} }}")
        }
    }
}

fn return_needs_arena(ty: &ResolvedTypeRef, ir: &ValidatedIr) -> bool {
    match ty {
        ResolvedTypeRef::AbiType(AbiBuiltin::StringView) => true,
        ResolvedTypeRef::UserDefined(name) if array_element_name(name).is_some() => true,
        ResolvedTypeRef::UserDefined(name) if ir.enums.iter().any(|item| item.name == *name) => {
            false
        }
        ResolvedTypeRef::UserDefined(name) => ir
            .types
            .iter()
            .find(|item| item.name == *name)
            .is_some_and(|item| {
                item.fields
                    .iter()
                    .any(|field| return_needs_arena(&field.ty, ir))
            }),
        ResolvedTypeRef::Primitive(_)
        | ResolvedTypeRef::AbiType(AbiBuiltin::Buffer)
        | ResolvedTypeRef::AbiType(AbiBuiltin::Ptr)
        | ResolvedTypeRef::AbiType(AbiBuiltin::Void) => false,
    }
}

fn resolved_type_ref_for_name(name: &str) -> ResolvedTypeRef {
    if let Some(primitive) = PrimitiveType::parse(name) {
        return ResolvedTypeRef::Primitive(primitive);
    }
    if let Some(abi) = AbiBuiltin::parse(name) {
        return ResolvedTypeRef::AbiType(abi);
    }
    ResolvedTypeRef::UserDefined(name.to_owned())
}

fn canonical_contract_diagnostic_name(name: &str) -> &str {
    name.split_once('@').map_or(name, |(contract, _)| contract)
}

fn generate_internal_factory_registry(out: &mut String, ir: &ValidatedIr) {
    let providers: Vec<GuestProvider<'_>> = guest_providers(ir);
    out.push_str("/// A thread-safe factory retained by this internal plugin bundle.\n");
    out.push_str("pub struct InternalProviderFactory<T: ?Sized> {\n");
    out.push_str("    factory: Arc<dyn Fn() -> Box<T> + Send + Sync + 'static>,\n");
    out.push_str("}\n\n");
    out.push_str("impl<T: ?Sized> InternalProviderFactory<T> {\n");
    out.push_str(
        "    /// Retain an ordinary function or captured closure for repeated instance creation.\n",
    );
    out.push_str("    pub fn new<F>(factory: F) -> Self\n    where\n");
    out.push_str("        F: Fn() -> Box<T> + Send + Sync + 'static,\n");
    out.push_str("    {\n        Self { factory: Arc::new(factory) }\n    }\n\n");
    out.push_str("    fn create(&self) -> Box<T> {\n        (self.factory)()\n    }\n");
    out.push_str("}\n\n");
    out.push_str("/// Ordinary provider factories for this internal plugin bundle.\n");
    out.push_str("pub struct InternalProviders {\n");
    for provider in &providers {
        let lower: String = internal_provider_symbol(provider);
        let trait_name: String = contract_name_to_struct(&provider.contract.name);
        out.push_str(&format!(
            "    pub {lower}: InternalProviderFactory<dyn super::guest_contracts::{trait_name}>,\n"
        ));
    }
    out.push_str("}\n\n");
    for provider in &providers {
        ...
        let domain_trait: String = contract_name_to_struct(&provider.contract.name);
        out.push_str(&format!(
            "unsafe fn internal_provider_factory_{lower}(adapter_context: *mut c_void, _host_ctx: HostContext) -> Option<Box<dyn super::guest_contracts::{domain_trait}>> {{\n"
        ));
        out.push_str("    if adapter_context.is_null() {\n        return None;\n    }\n");
        out.push_str(&format!(
            "    let providers: &InternalProviders = unsafe {{ &*(adapter_context as *const InternalProviders) }};\n    Some(providers.{lower}.create())\n}}\n\n"
        ));
    }
}
...
fn generate_guest_contract_interface(
    ...
    emit_guest_instance_machinery(
        ...
        GuestInstanceMachinery {
            ...
            contract_id_const: &format!("{upper}_CONTRACT_ID"),
            embedded_factory: internal_mode.map(|_| internal_factory_symbol(&lower)),
            profile_adapter: domain_api.then(|| profile_adapter_name(&lower)),
            domain_trait: domain_api.then(|| format!("super::guest_contracts::{struct_name}")),
        },
    )?;
...
/// Inputs that identify a plugin implementation and its generated interface.
struct GuestPluginInterfaceConfig<'a> {
    plugin_name: &'a str,
    plugin_symbol: &'a str,
    contract: &'a ResolvedContract,
    is_native: bool,
    emitted_consts: &'a mut HashSet<String>,
    internal_mode: Option<InternalBindingMode>,
    domain_api: bool,
}
...
fn generate_guest_plugin_interface(
    out: &mut String,
    config: GuestPluginInterfaceConfig<'_>,
) -> Result<(), PolyplugcError> {
    let GuestPluginInterfaceConfig {
        plugin_name,
        plugin_symbol,
        contract,
        is_native,
    ...
    let plugin_upper: String = plugin_symbol.to_uppercase().replace('.', "_");
    let plugin_lower: String = plugin_symbol.to_lowercase().replace('.', "_");
    ...
    emit_guest_instance_machinery(
        out,
        GuestInstanceMachinery {
            prefix_upper: &plugin_upper,
            lower: &plugin_lower,
            trait_name: &trait_name,
            state_struct: &state_struct,
            contract_id_const: &format!("{contract_upper}_CONTRACT_ID"),
            embedded_factory: internal_mode.map(|_| internal_factory_symbol(&plugin_lower)),
            profile_adapter: domain_api.then(|| profile_adapter_name(&plugin_lower)),
            domain_trait: domain_api.then(|| format!("super::guest_contracts::{struct_name}")),
        },
    )?;
...
fn internal_factory_symbol(provider: &str) -> String {
    format!("internal_provider_factory_{provider}")
}
...
struct GuestInstanceMachinery<'a> {
    ...
    trait_name: &'a str,
    state_struct: &'a str,
    contract_id_const: &'a str,
    embedded_factory: Option<String>,
    profile_adapter: Option<String>,
    domain_trait: Option<String>,
}

fn emit_guest_instance_machinery(
    out: &mut String,
    machinery: GuestInstanceMachinery<'_>,
) -> Result<(), PolyplugcError> {
    let GuestInstanceMachinery {
        prefix_upper,
        lower,
        trait_name,
        state_struct,
    ...
    let external_factory: bool = embedded_factory.is_none();
    let factory_name: String =
        embedded_factory.unwrap_or_else(|| format!("polyplug_create_{lower}"));
    let factory_trait = domain_trait.as_deref().unwrap_or(trait_name);
    let external_implementation_name = if domain_trait.is_some() {
        "domain_implementation"
    } else {
        "implementation"
    };
    let external_adapter = profile_adapter.as_deref().map_or_else(String::new, |adapter| {
        format!(
            "\nlet implementation: Box<dyn {trait_name}> = Box::new({adapter}::new(domain_implementation, host_ctx));"
        )
    });
        ...
        let extern_block: RustExternBlock = RustExternBlock {
            ...
            items: vec![RustFunction {
                ...
                docs: Some(vec![
                    ...
                    format!(
                        "`pub fn {factory_name}(host: HostContext) -> Box<dyn {factory_trait}> {{ ... }}`"
                    ),
                ]),
                comments: Vec::new(),
            }],
    ...
    let create_body: String = if !external_factory {
        let implementation_start: String = match profile_adapter.as_deref() {
            Some(_) => "let domain_implementation =".to_owned(),
            None => format!("let implementation: Box<dyn {trait_name}> ="),
        };
        let implementation_end: String = match profile_adapter.as_deref() {
            Some(adapter) => format!(
                "\nlet implementation: Box<dyn {trait_name}> = Box::new({adapter}::new(domain_implementation, host_ctx));"
            ),
            None => String::new(),
        ...
        format!(
            "if out_instance.is_null() {{\n\
             ...
             // SAFETY: adapter_context points at the runtime-owned internal-plugin provider roots.\n\
        ...
        format!(
            "if out_instance.is_null() {{\n\
             ...
             // for create_instance; it stays valid for the runtime's lifetime.\n\
             let host_ctx: HostContext = unsafe {{ HostContext::new(host) }};\n\
             let {external_implementation_name}: Box<dyn {factory_trait}> =
             match std::panic::catch_unwind(core::panic::AssertUnwindSafe(|| {{
    ...
    let create_adapter_context_name: &str = if !external_factory {
        "adapter_context"
    } else {
        "_adapter_context"
    };
...
fn generate_guest_init_file(
    out: &mut String,
    ir: &ValidatedIr,
    internal_bundle_name: Option<&str>,
    internal_mode: Option<InternalBindingMode>,
) -> Result<(), PolyplugcError> {
    match (internal_bundle_name, internal_mode) {
        (Some(bundle_name), Some(InternalBindingMode::Profile)) => {
            generate_internal_profile_guest_init_file(out, ir, bundle_name)?;
            return Ok(());
        }
        (None, None) => {}
        _ => unreachable!("internal binding mode and bundle name must be paired"),
    }
...
fn generate_internal_profile_manifest(ir: &ValidatedIr) -> Result<String, PolyplugcError> {
    let bundle = ir
        .bundle
        .as_ref()
        .ok_or_else(|| PolyplugcError::ValidationFailed {
            message: "Rust internal bindings require a bundle manifest".to_owned(),
        })?;
    let provides: BTreeSet<String> = bundle
        .plugins
        .iter()
...
fn generate_internal_profile_guest_init_file(
    out: &mut String,
    ir: &ValidatedIr,
    bundle_name: &str,
) -> Result<(), PolyplugcError> {
    ...
    let providers: Vec<GuestProvider<'_>> = guest_providers(ir);
    let manifest_toml: String = generate_internal_profile_manifest(ir)?;
    out.push_str(&rust_use_block(&[
        &["core::ffi::c_void", "std::sync::Arc"],
        &[
            "polyplug::Runtime",
            "polyplug::error::LoaderError",
            "polyplug::error::RuntimeError",
            "polyplug::runtime::RustGeneratedInternalPlugin",
            "polyplug::runtime::RustGeneratedInternalPluginRegistrar",
            "polyplug_abi::PluginDescriptor",
            "polyplug_abi::StringView",
            "polyplug_abi::Version",
        ...
        &["super::interfaces::InternalProviders"],
    ]));
    out.push('\n');
    out.push_str(&format!(
        "pub const INTERNAL_BUNDLE_NAME: &str = {bundle_name:?};\n\
         pub const INTERNAL_BUNDLE_ID: u64 = {};\n\n",
        bundle.bundle_id
    ));
    out.push_str(&format!(
        "const INTERNAL_MANIFEST: &str = {manifest_toml:?};\n\n"
    ));
    out.push_str("struct InternalRegistration {\n    providers: Box<InternalProviders>,\n}\n\n");
    out.push_str("impl RustGeneratedInternalPlugin for InternalRegistration {\n");
    out.push_str("    fn manifest(&self) -> ManifestData {\n");
    out.push_str("        ManifestData::parse_from_str(INTERNAL_MANIFEST)\n");
    out.push_str("            .expect(\"generated internal manifest is valid\")\n");
    out.push_str("    }\n\n");
    out.push_str("    fn stage(&self, registrar: &mut RustGeneratedInternalPluginRegistrar) -> Result<(), RuntimeError> {\n");
    out.push_str("        let adapter_context: *mut c_void = (&*self.providers as *const InternalProviders).cast_mut().cast();\n");
    for provider in &providers {
        let plugin_name: String = guest_provider_name(provider);
        let interface_upper: String = internal_provider_upper(provider);
        let contract = provider.contract;
        let contract_diagnostic_name: &str = canonical_contract_diagnostic_name(&contract.name);
        out.push_str(&format!(
            "        let mut interface_{interface_upper} = super::interfaces::{interface_upper}_INTERFACE;\n\
             ...
             \x20       }};\n\
             \x20       registrar.register_contract(&descriptor_{interface_upper}, &interface_{interface_upper})?;\n",
            contract_diagnostic_name,
    ...
    out.push_str("        Ok(())\n    }\n}\n\n");
    out.push_str("pub struct Registration {\n    pub bundle_id: BundleId,\n");
    for provider in &providers {
        let field: String = internal_provider_symbol(provider);
        let caller: String = contract_name_to_struct(&provider.contract.name);
        out.push_str(&format!(
            "    pub {field}: super::super::host::host_callers::{caller},\n"
        ));
    }
    out.push_str("}\n\n");
    out.push_str("/// Register this internal plugin and return its exact typed caller set.\n");
    out.push_str(
        "pub fn register(\n    runtime: Arc<Runtime>,\n    providers: InternalProviders,\n) -> Result<Registration, RuntimeError> {\n\
         \x20   let published = runtime.register_generated_internal_plugin(InternalRegistration { providers: Box::new(providers) })?;\n\
         \x20   let mut handles = published.handles.into_iter();\n\
         \x20   let callers = (|| -> Result<Registration, RuntimeError> {\n",
    );
    for provider in &providers {
        let field: String = internal_provider_symbol(provider);
        let caller: String = contract_name_to_struct(&provider.contract.name);
        out.push_str(&format!(
            "    let {field} = super::super::host::host_callers::{caller}::new(\n\
             \x20       handles.next().expect(\"runtime returned one handle per generated provider\"),\n\
             \x20       Arc::clone(&runtime),\n\
             \x20   ).ok_or_else(|| RuntimeError::Loader(LoaderError::InitFailed {{\n\
             \x20       bundle: INTERNAL_BUNDLE_NAME.to_owned(),\n\
             \x20       error: \"generated caller instance creation failed\".to_owned(),\n\
             \x20   }}))?;\n"
        ));
    }
    out.push_str("        debug_assert!(handles.next().is_none());\n");
    out.push_str("        Ok(Registration {\n            bundle_id: published.bundle_id,\n");
    for provider in &providers {
        let field: String = internal_provider_symbol(provider);
    ...
    out.push_str(
        "        })\n\
         \x20   })();\n\
         \x20   match callers {\n\
         \x20       Ok(callers) => Ok(callers),\n\
         \x20       Err(error) => {\n\
         \x20           runtime.unload_bundle(published.bundle_id)?;\n\
         \x20           Err(error)\n\
         \x20       }\n\
         \x20   }\n\
         }\n",
...
fn fn_needs_domain_argument_arena(func: &ResolvedFunction) -> bool {
    func.params
        .iter()
        .any(|param| matches!(param.ty, ResolvedTypeRef::UserDefined(_)))
}

fn domain_type_contains_buffer(ty: &ResolvedTypeRef, ir: &ValidatedIr) -> bool {
    match ty {
        ResolvedTypeRef::AbiType(AbiBuiltin::Buffer) => true,
        ResolvedTypeRef::UserDefined(name) if let Some(element) = array_element_name(name) => {
            domain_type_contains_buffer(&resolved_type_ref_for_name(element), ir)
        }
        ResolvedTypeRef::UserDefined(name) => ir
            .types
            .iter()
            ...
            .is_some_and(|item| {
                item.fields
                    .iter()
                    .any(|field| domain_type_contains_buffer(&field.ty, ir))
            }),
        _ => false,
    }
}
...
fn generate_host_contract_caller(
    ...
    let needs_arena: bool = contract_needs_arena(contract)
        || domain_ir.is_some()
            && contract
                .functions
                .iter()
                .any(fn_needs_domain_argument_arena);
    ...
    out.push_str("        let host_api: &HostApi = unsafe { &*host };\n");
    out.push_str("        let revision = self.live_revision();\n");
    out.push_str("        let interface = if revision != self.cached_revision || self.interface.is_null() {\n");
    out.push_str(
    ...
    out.push_str("                self.cached_revision = revision;\n");
    out.push_str("                return;\n");
    out.push_str("            }\n");
    ...
    out.push_str(
        "    /// Re-resolve the cached interface after the registry changed under us.\n\
         ...
         \x20   ///\n\
         \x20   /// If an unrelated registry change left the resolved interface unchanged, the\n\
         \x20   /// caller keeps its instance and only refreshes the cached revision. Otherwise\n\
         \x20   /// the old instance is ABANDONED, never destroyed: after a reload its interface\n\
         \x20   /// and the guest-side state it created are already epoch-reclaimed, so calling\n\
         \x20   /// the dead interface's `destroy_instance` would be undefined behaviour. The\n\
         \x20   /// runtime reclaims the old instance's backing as part of the reload, then a\n\
         \x20   /// fresh instance is created on the new interface.\n",
    );
    ...
    out.push_str("        }\n");
    out.push_str("        if interface == self.interface {\n");
    out.push_str("            self.cached_revision = self.live_revision();\n");
    ...
    for func in &contract.functions {
        generate_host_fn_caller(out, func, contract, &struct_name, domain_ir)?;
    }
...
fn generate_host_fn_caller(
    ...
    let sig_params: String = domain_ir.map_or_else(
        || build_sig_params(func),
        |ir| build_domain_sig_params(func, ir),
    );

    let ret_type: String = match (&func.returns, domain_ir) {
        (Some(ty), Some(ir)) => host_domain_type_name(ty, ir),
        (Some(ty), None) => rust_type_name(ty),
        (None, _) => "()".to_owned(),
    };
    ...
    out.push_str("        }\n");
    if domain_ir.is_some() && needs_arena {
        out.push_str(
            "        // Rewind caller-owned argument and return storage before lowering this call.\n",
        );
        out.push_str("        self.arena.reset();\n");
    }

    if let Some(ir) = domain_ir {
        emit_host_domain_args_setup(out, func, contract_struct, ir);
    } else {
    ...
    emit_native_vm_dispatch(out, fn_id, needs_arena, domain_ir.is_some());
...
fn emit_native_vm_dispatch(
    out: &mut String,
    fn_id: u32,
    needs_arena: bool,
    arena_reset_before_lowering: bool,
) {
    ...
    if needs_arena && !arena_reset_before_lowering {
        out.push_str(
...
fn host_domain_type_name(ty: &ResolvedTypeRef, ir: &ValidatedIr) -> String {
    match ty {
        ResolvedTypeRef::UserDefined(name) => {
            if let Some(element) = array_element_name(name) {
                return format!(
                    "Vec<{}>",
                    host_domain_type_name(&resolved_type_ref_for_name(element), ir)
                );
            }
            format!("super::domain::{name}")
        }
        _ => domain_type_name(ty, &ir.types, &ir.enums),
    }
}
...
fn emit_host_domain_args_setup(
    ...
    if func.params.is_empty() {
        ...
        let abi_type = rust_type_name(&param.ty);
        let expression = host_abi_from_domain_expr(&param.ty, &param.name, ir, false);
        out.push_str(&format!(
            "        let {}_abi: {abi_type} = {expression};\n        let args_ptr: *const () = &{}_abi as *const {abi_type} as *const ();\n",
            param.name, param.name
        ));
    } else {
        let pack_struct = arg_pack_struct_name(contract_struct, &func.name);
        out.push_str(&format!(
        ...
        for param in &func.params {
            out.push_str(&format!(
                "            {}: {},\n",
                param.name,
                host_abi_from_domain_expr(&param.ty, &param.name, ir, false)
            ));
        }
        out.push_str("        };\n");
        out.push_str(&format!(
...
fn host_abi_from_domain_expr(
    ...
    match ty {
        ...
        ResolvedTypeRef::AbiType(AbiBuiltin::Void) => "()".to_owned(),
        ResolvedTypeRef::AbiType(AbiBuiltin::StringView) => {
            format!("StringView {{ ptr: ({value}).as_ptr().cast_mut(), len: ({value}).len() }}")
        }
        ResolvedTypeRef::AbiType(AbiBuiltin::Buffer) => format!(
            "Buffer {{ ptr: ({value}).as_ptr().cast_mut(), len: ({value}).len(), cap: ({value}).len() }}"
        ),
        ResolvedTypeRef::UserDefined(name) if ir.enums.iter().any(|item| item.name == *name) => {
            let Some(enum_def) = ir.enums.iter().find(|item| item.name == *name) else {
                unreachable!("validated enum is present");
        ...
        ResolvedTypeRef::UserDefined(name) if let Some(element) = array_element_name(name) => {
            ...
            let abi_element = rust_type_name(&element_ty);
            format!(
                "{{ let values: Vec<{abi_element}> = ({value}).iter().map(|item| -> Result<_, ContractError> {{ Ok({}) }}).collect::<Result<Vec<_>, ContractError>>()?; let (items, len) = caller_arena_array(&mut self.arena, &values)?; super::types::{name} {{ items, len }} }}",
                host_abi_from_domain_expr(&element_ty, "item", ir, true)
            )
        }
        ResolvedTypeRef::UserDefined(name) => {
...
#![allow(dead_code)]\n#![allow(non_snake_case)]\n#![allow(non_camel_case_types)]\n#![allow(clippy::all)]\n#![allow(clippy::pedantic)]\n#![allow(clippy::nursery)]\n\n";
    ...
    let any_needs_arena: bool = peers.iter().any(|contract| {
        contract_needs_arena(contract)
            || domain_ir.is_some()
                && contract
                    .functions
                    .iter()
    ...
    let needs_domain_buffer = domain_ir.is_some_and(|domain| {
        peers.iter().any(|contract| {
            contract.functions.iter().any(|func| {
                func.params
                    .iter()
                    .any(|param| domain_type_contains_buffer(&param.ty, domain))
                    || func
                        .returns
                        .as_ref()
                        .is_some_and(|ty| domain_type_contains_buffer(ty, domain))
            })
        })
    });
    if needs_domain_buffer {
        external.push("polyplug_abi::Buffer");
    }
    let local_imports: &[&str] = &["super::types::*"];
    out.push_str(&rust_use_block(&[
        &["core::ffi::c_void"],
    ...
    if domain_ir.is_some() {
        out.push_str(
            "fn peer_caller_arena_array<T>(arena: &mut CallArena, values: &[T]) -> Result<(u64, u64), PeerCallError> {\n\
             \x20   if values.is_empty() {\n\
             \x20       return Ok((0, 0));\n\
             \x20   }\n\
             \x20   let bytes = values.len().checked_mul(core::mem::size_of::<T>()).ok_or_else(|| PeerCallError::new(AbiErrorCode::Generic))?;\n\
             \x20   let ptr = arena.alloc(bytes, core::mem::align_of::<T>()).cast::<T>();\n\
             \x20   if ptr.is_null() {\n\
             \x20       return Err(PeerCallError::new(AbiErrorCode::Generic));\n\
             \x20   }\n\
             \x20   unsafe { core::ptr::copy_nonoverlapping(values.as_ptr(), ptr, values.len()); }\n\
             \x20   Ok((ptr as u64, values.len() as u64))\n\
             }\n\n",
    ...
    for contract in peers {
        ...
        generate_peer_caller(&mut out, contract, min_ver, domain_ir);
    }
...
fn generate_peer_caller(
    ...
    // Methods
    for func in &contract.functions {
        generate_peer_fn_caller(out, func, &struct_name, domain_ir);
    }
...
fn generate_peer_fn_caller(
    ...
    if let Some(ir) = domain_ir {
        emit_peer_domain_args_setup(out, func, struct_name, ir);
    } else {
...
fn emit_peer_domain_args_setup(
    out: &mut String,
    func: &ResolvedFunction,
    contract_struct: &str,
    ir: &ValidatedIr,
    ...
    if func.params.is_empty() {
        ...
        let param = &func.params[0];
        let abi_type = rust_type_name(&param.ty);
        let expression = peer_abi_from_domain_expr(&param.ty, &param.name, ir, false);
        out.push_str(&format!(
            "        let {}_abi: {abi_type} = {expression};\n        let args_ptr: *const () = &{}_abi as *const {abi_type} as *const ();\n",
            param.name, param.name
        ));
        ...
        for param in &func.params {
            out.push_str(&format!(
                "            {}: {},\n",
                param.name,
                peer_abi_from_domain_expr(&param.ty, &param.name, ir, false)
            ));
        }
        out.push_str("        };\n");
        out.push_str(&format!(
...
fn peer_abi_from_domain_expr(
    ...
    match ty {
        ...
        ResolvedTypeRef::UserDefined(name) if let Some(element) = array_element_name(name) => {
            ...
            let abi_element = rust_type_name(&element_ty);
            format!(
                "{{ let values: Vec<{abi_element}> = ({value}).iter().map(|item| -> Result<_, PeerCallError> {{ Ok({}) }}).collect::<Result<Vec<_>, PeerCallError>>()?; let (items, len) = peer_caller_arena_array(&mut self.arena, &values)?; super::types::{name} {{ items, len }} }}",
                peer_abi_from_domain_expr(&element_ty, "item", ir, true)
            )
        }
        ResolvedTypeRef::UserDefined(name) => {
...
fn generate_guest_mod_rs(
    ...
    if internal_mode == Some(InternalBindingMode::Profile) || domain_api {
        ...
        out.push_str("mod contracts;\n");
        out.push_str("pub(crate) mod interfaces;\n");
    } else {
...
mod tests {
    ...
    fn no_peer_callers_without_dependencies() {
        ...
        let mut files: GeneratedFiles = GeneratedFiles::default();
        generator
            .generate_guest(&ir_no_bundle, &OutputLayout::unified(), &mut files)
            .expect("generate_guest");
    ...
    #[test]
    fn internal_domain_adapters_map_nominal_enums_in_both_directions() {
        let ir = ValidatedIr {
            types: vec![],
            enums: vec![EnumDef {
                name: "Mode".to_owned(),
        ...
        assert_eq!(
            domain_from_abi_expr(&mode, "value", &ir),
            "match *(value) { super::types::Mode::Cold => super::domain::Mode::Cold, super::types::Mode::Hot => super::domain::Mode::Hot }"
        );
        assert_eq!(
            abi_domain_value_expr(&mode, "value", &ir),
            "match value { super::domain::Mode::Cold => super::types::Mode::Cold, super::domain::Mode::Hot => super::types::Mode::Hot }"
        );
    }

    #[test]
    fn array_element_resolver_preserves_builtin_and_nested_wrapper_kinds() {
        assert!(matches!(
            resolved_type_ref_for_name("u32"),
            ResolvedTypeRef::Primitive(PrimitiveType::U32)
        ));
        assert!(matches!(
            resolved_type_ref_for_name("StringView"),
            ResolvedTypeRef::AbiType(AbiBuiltin::StringView)
        ));
        assert!(matches!(
            resolved_type_ref_for_name("Buffer"),
            ResolvedTypeRef::AbiType(AbiBuiltin::Buffer)
        ));
        assert!(matches!(
            resolved_type_ref_for_name("ArrayOf_ArrayOf_u32"),
            ResolvedTypeRef::UserDefined(name) if name == "ArrayOf_ArrayOf_u32"
        ));
    }
    #[test]
    fn split_host_buffer_returns_free_only_native_allocations() {
        let nested = ResolvedType {
        ...
        RustGenerator
            .generate_host(&ir, &layout, &mut files)
            .expect("generate split host");
        let callers = &files
            .files
            .iter()
        ...
        assert!(
            ...
            "the ownership discriminator must be emitted after dispatch: {callers}"
        );
        assert_eq!(
            callers
                .matches("if returned_from_native && !buffer.ptr.is_null()")
                .count(),
            2,
            "nested struct and array Buffer conversions must guard deallocation by native ownership: {callers}"
        );
        assert!(
            callers.contains(".as_slice() }.iter().map"),
            "nested arrays must recursively lower their Buffer elements: {callers}"
        );
        assert!(
            !callers.contains("if !buffer.ptr.is_null() { let host = self.host()"),
            "VM CallArena-backed buffers must not be freed as host allocations: {callers}"
        );
    }

    #[test]
    fn guest_contract_only_layout_keeps_host_caller_signature_unified() {
        let ir = ValidatedIr {
            types: vec![],
            enums: vec![],
            contracts: vec![ResolvedContract {
                name: "counter.increment".to_owned(),
                contract_id: 8,
                version: Version {
                    major: 1,
                    minor: 0,
        ...
        RustGenerator
            .generate_host(&ir, &OutputLayout::unified(), &mut unified)
            .expect("generate unified host");
        let mut guest_split = GeneratedFiles::default();
        RustGenerator
            .generate_host(
                &ir,
                &OutputLayout {
                    ...
                    guest_contracts: OutputDestination::Emit {
                        root: PathBuf::from("guest-contracts"),
                        import: ValidatedImport::parse(Lang::Rust, "app::guest_contracts")
                            .expect("valid guest-contract import"),
                    },
                },
                &mut guest_split,
            )
            .expect("generate guest-contract-only split host");

        let unified_callers = &unified
            .files
            .iter()
            .find(|file| file.path == Path::new("host/host_callers.rs"))
            .expect("unified host callers")
            .content;
        let split_callers = &guest_split
            .files
            .iter()
            .find(|file| file.path == Path::new("host/host_callers.rs"))
            .expect("guest-contract-only host callers")
            .content;
        assert_eq!(
    ...
    #[test]
    fn internal_profile_contract_diagnostic_name_excludes_version_suffix() {
        assert_eq!(
            canonical_contract_diagnostic_name("platform.Plugin@1"),
            "platform.Plugin"
        );
        assert_eq!(
            canonical_contract_diagnostic_name("platform.Plugin"),
            "platform.Plugin"
        );
    }
}
