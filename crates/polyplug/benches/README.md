...
> regenerated by `just bench-charts`). The **guest** chart reads every bar live
> from criterion (native from `counter_inc`, each VM from its loader's warm
> dispatch bench), so run the loader crates too. The **host** chart's bars are the
> measured FFI figures documented there (each needs its language runtime).
...
> owned copy). Charts: `docs/assets/benches/{counter_inc,dispatch_by_shape,
> payload_scaling,marshalling,amortization,cross_lang_guest,cross_lang_host}.svg`.
