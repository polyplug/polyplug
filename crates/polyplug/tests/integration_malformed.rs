...
fn make_tmpdir(name: &str) -> PathBuf {
    let base: PathBuf = env::temp_dir().join(format!("polyplug_test_{name}"));
...
fn test_truncated_so() {
    ...
    let rc: AbiError = load_bundle_path(host, dir.to_str().expect("valid utf8 path"));
    assert_ne!(
        rc.code,
...
fn test_wrong_magic_bytes() {
    ...
    let rc: AbiError = load_bundle_path(host, dir.to_str().expect("valid utf8"));
    assert_ne!(
        rc.code,
        AbiErrorCode::Ok as u32,
