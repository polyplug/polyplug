...
fn main() {
    ...
    println!("cargo:rustc-env=NO_INIT_PLUGIN_SO={}", no_init_plugin_so.display());

    // DEPENDER_PLUGIN_SO — dependency test plugin
    let depender_plugin_so: PathBuf = fixtures_dir.join(format!("libdepender_plugin.{}", plugin_ext));
    println!("cargo:rustc-env=DEPENDER_PLUGIN_SO={}", depender_plugin_so.display());
    ...
    println!("cargo:rustc-env=RELOAD_PLUGIN_V1_SO={}", reload_v1_so.display());
    let reload_v2_so: PathBuf = fixtures_dir.join(format!("libreload_plugin_v2.{}", plugin_ext));
    println!("cargo:rustc-env=RELOAD_PLUGIN_V2_SO={}", reload_v2_so.display());

    // TEST_PLUGIN_CPP_SO — C++ test plugin
    let test_plugin_cpp_so: PathBuf = fixtures_dir.join(format!("libtest_plugin_cpp.{}", plugin_ext));
    println!("cargo:rustc-env=TEST_PLUGIN_CPP_SO={}", test_plugin_cpp_so.display());

    // TEST_PLUGIN_CPP_THROW_SO — C++ throw test plugin
    let test_plugin_cpp_throw_so: PathBuf = fixtures_dir.join(format!("libtest_plugin_cpp_throw.{}", plugin_ext));
    println!("cargo:rustc-env=TEST_PLUGIN_CPP_THROW_SO={}", test_plugin_cpp_throw_so.display());
    ...
    println!("cargo:rustc-env=TEST_PLUGIN_DIR={}", test_plugin_dir.display());

    // NO_INIT_PLUGIN_DIR — plugin without polyplug_init directory
    let no_init_plugin_dir: PathBuf = fixtures_dir.join("no_init_plugin");
    println!("cargo:rustc-env=NO_INIT_PLUGIN_DIR={}", no_init_plugin_dir.display());
