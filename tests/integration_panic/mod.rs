...
fn test_panic_returns_abi_error_panic() {
    ...
    // CARGO_MANIFEST_DIR = crates/polyplug-runtime; workspace root is two up.
    ...
    // Only depend on polyplug-guest; polyplug-runtime is an indirect dep.
    // We do NOT add polyplug-runtime as a direct dep to avoid duplicate
    // `polyplug_abi_version` symbol (it is defined in polyplug-runtime/src/lib.rs).
    ...
    // We do NOT define polyplug_abi_version — it comes from polyplug-runtime rlib.
