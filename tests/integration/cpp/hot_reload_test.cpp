...
void test_reload_phase_type_constants() {
    ...
    ASSERT_EQUALS(0U, static_cast<uint32_t>(ReloadPhaseType_Preparing), "TYPE_PREPARING should be 0");
    ASSERT_EQUALS(1U, static_cast<uint32_t>(ReloadPhaseType_Reloaded), "TYPE_RELOADED should be 1");
    ASSERT_EQUALS(2U, static_cast<uint32_t>(ReloadPhaseType_Failed), "TYPE_FAILED should be 2");
}
...
void test_reload_phase_struct() {
    ...
    ReloadPhase phase;
    phase.type = ReloadPhaseType_Preparing;
    ...
    ASSERT_EQUALS(ReloadPhaseType_Preparing, phase.type, "phase.type should be TYPE_PREPARING");
    ...
    ASSERT_EQUALS(2U, phase.retry_count, "phase.retry_count should be 2");
    ...
    phase_defaults.type = ReloadPhaseType_Reloaded;
    phase_defaults.bundle_id = 999ULL;
    phase_defaults.bundle_name = StringView{reinterpret_cast<const uint8_t*>("MyBundle"), 8};
    phase_defaults.retry_count = 0;
    phase_defaults.reason = StringView{nullptr, 0};

    ASSERT_EQUALS(ReloadPhaseType_Reloaded, phase_defaults.type, "phase_defaults.type should be TYPE_RELOADED");
    ASSERT_EQUALS(999ULL, phase_defaults.bundle_id, "phase_defaults.bundle_id should be 999");
    ASSERT_EQUALS(0U, phase_defaults.retry_count, "phase_defaults.retry_count should be 0");
    ...
    failed_phase.type = ReloadPhaseType_Failed;
    ...
    ASSERT_EQUALS(ReloadPhaseType_Failed, failed_phase.type, "failed_phase.type should be TYPE_FAILED");
