...
use polyplug::ffi::{
    ...
    polyplug_runtime_free, polyplug_runtime_new, polyplug_runtime_register_loader,
};
...
fn create_runtime_with_native_loader() -> *mut OpaqueRuntime {
    ...
    let native_loader: *mut std::ffi::c_void =
        unsafe { polyplug_native_loader_create(core::ptr::null::<PolyplugNativeConfig>() as *mut _) };
    assert!(!native_loader.is_null(), "native loader create failed");

    let reg_result: u32 =
        unsafe { polyplug_runtime_register_loader(rt, native_loader) };
    assert_eq!(
        reg_result, 0,
