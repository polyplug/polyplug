...
unsafe extern "C" fn stub_list_bundles(_this: *const HostApi, out_bundles: *mut Array<BundleId>) {
...
/// Stub get_dependencies — writes an empty array.
...
fn test_rust_host_rust_guest() {
    ...
    let interface: &GuestContractInterface = unsafe { &*interface_ptr };
    // SAFETY: functions[0] is the `add` wrapper with signature extern "C" fn(GuestContractInstance, *const (), *mut ()) -> AbiError.
    let fn_ptr: *const () = unsafe { *interface.dispatch.native.functions.add(0) };
