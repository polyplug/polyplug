//! Integration test: Python VM guests have **real per-instance state**.
//!
//! # Why this exists
//!
//! Python contracts dispatch through the `polyplug_python` VM loader. Before the
...
//! "instance" of a Python contract shared one implementation object. That is a
//! latent correctness bug for any stateful Python plugin.
//!
//! The loader now owns per-instance state: `create_instance` calls the contract's
//! author factory (carried through the registration's `factory` entry and reached
//! via the interface's `loader_data`) to build a fresh impl, mints a non-zero
//! instance id, and keys the impl in a per-contract registry; dispatch resolves the
//! impl from the instance handle and passes it as the callable's first argument;
//! `destroy_instance` drops it. A null instance handle resolves to a per-contract
//! default impl built once at load (stateless / low-level paths).
//!
...
const BUNDLE_NAME: &str = "iso_counter";
...
/// Write the stateful `iso.Counter@1` Python bundle into `tmp/<dir_name>`:
/// generate the guest glue, write the hand-written impl whose counter lives on the
...
fn write_counter_bundle(tmp: &Path, dir_name: &str) -> PathBuf {
    ...
    let entry_py: &str = "from generated.guest.contracts import set_counter_factory, polyplug_init\n\
         \n\
         \n\
         class CounterImpl:\n\
         ...
         \x20       self._count = 0\n\
         \n\
         \x20   def inc(self) -> int:\n\
         \x20       self._count += 1\n\
         \x20       return self._count\n\
         \n\
...
/// prepends it to `sys.path`). Copies the real SDK so the helpers match the
/// generated glue exactly. Mirrors `integration_python_generated_impl_isolation`.
...
fn resolve_counter(rt: &Runtime) -> *const GuestContractInterface {
    ...
    // SAFETY: the interface is live for the loaded bundle; the python module stays
...
#[test]
fn two_instances_of_one_python_contract_have_independent_state() {
    const INC_FN: u32 = 0;
    const GET_FN: u32 = 1;
