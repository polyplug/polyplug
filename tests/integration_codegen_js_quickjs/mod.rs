//! Integration test: run polyplugc to generate js-quickjs bindings and assert all expected
//! files are present.
//!
//! This test crate is the crate root for the `integration_codegen_js_quickjs` test binary.
...
/// Run `polyplugc generate --bundle <bundle_toml> --lang js-quickjs --out <out_dir>`.
/// Returns the `Output` for inspection.
fn run_polyplugc_js_quickjs(bundle_toml: &Path, out_dir: &Path) -> Output {
...
fn test_generate_js_quickjs_files_exist() {
    ...
    // ── 1. Run polyplugc to generate js-quickjs bindings ──────────────────────
    ...
    println!(
        "test_generate_js_quickjs_files_exist: all {} files present in {} ✓",
        expected_files.len(),
        out_dir.display()
    );
}
