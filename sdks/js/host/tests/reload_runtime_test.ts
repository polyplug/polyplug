...
//   deno test -A sdks/js/host/tests/reload_runtime_test.ts

import { assert, assertEquals } from "jsr:@std/assert";
import { openPolyplug, runtimeNew, ReloadPhase } from "../mod.js";
import { registerNativeLoader } from "../../loaders/native/mod.ts";

const POLYPLUG_LIB: string = Deno.env.get("POLYPLUG_LIB") ?? Deno.env.get("POLYPLUG_SO") ?? "";
if (!POLYPLUG_LIB) {
    console.error(
...
Deno.test("onReload fires with real phase data on a real runtime reload", () => {
    requireFixture(`${V1_DIR}/manifest.toml`);
    requireFixture(`${V1_DIR}/libreload_plugin_v1.so`);
    requireFixture(V2_SO);

    const lib = openPolyplug(POLYPLUG_LIB);
    try {
