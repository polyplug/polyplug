// Test: the host-contract PROVIDER (buildHostContractInterface) gives each
// instance its own state.
//
// # Why this exists
//
// A host contract registered by a JS host is provided through
// `buildHostContractInterface`, which builds a real C `HostContractInterface`
// with native dispatch backed by host callbacks. For a non-singleton contract
// the runtime calls `create_instance` once per caller; each call must build a
// FRESH implementation and dispatch must route to the right one by the instance
// handle. This test drives the generated C function pointers directly (the exact
// path the runtime takes) and asserts two instances never share state.
//
// The raw-FFI work goes through the runtime-agnostic seam (`getBackend()`), so
// this body has no `Deno.*` and runs identically once Node/Bun backends land.
//
// This surface has no cargo coverage (the JS host SDK runs only on a JS runtime),
// so this test is the per-instance floor; the end-to-end path is covered by the
...
import { test } from "../../testing/harness.ts";

const HOST_CONTRACT_INSTANCE_STRUCT = { struct: ["pointer"] } as const;

const CREATE_DEF: FfiSymbolDef = {
    parameters: ["pointer", "pointer", "pointer"],
    result: "void",
};
const DESTROY_DEF: FfiSymbolDef = {
    parameters: ["pointer", HOST_CONTRACT_INSTANCE_STRUCT],
    result: "void",
};
const DISPATCH_DEF: FfiSymbolDef = {
    parameters: [HOST_CONTRACT_INSTANCE_STRUCT, "pointer", "pointer", "pointer"],
    result: "void",
};

function ptrField(view: FfiPointerView, offset: number): PolyPtr {
    const p: PolyPtr = getBackend().pointerCreate(view.getBigUint64(offset));
    if (p === null) {
...
test("host-contract provider gives each instance independent state", () => {
    ...
    const ifaceView: FfiPointerView = backend.pointerView(interfacePtr);
    const createFn: FfiFunction = backend.prepareFunction(
        ptrField(ifaceView, HOST_CONTRACT_INTERFACE_CREATE_INSTANCE_OFFSET),
        CREATE_DEF,
    );
    const destroyFn: FfiFunction = backend.prepareFunction(
        ptrField(ifaceView, HOST_CONTRACT_INTERFACE_DESTROY_INSTANCE_OFFSET),
        DESTROY_DEF,
    );
    const fnsArrayPtr: PolyPtr = ptrField(
        ifaceView,
        HOST_CONTRACT_INTERFACE_DISPATCH_OFFSET + NATIVE_DISPATCH_FUNCTIONS_OFFSET,
    );
    const incFn: FfiFunction = backend.prepareFunction(
        ptrField(backend.pointerView(fnsArrayPtr), 0),
        DISPATCH_DEF,
    );
    ...
    const createInstance = (): Uint8Array => {
        ...
        createFn.call(null, null, backend.pointerOf(inst));
        return inst;
    };
    const errBuf = new Uint8Array(24);
    const errPtr: PolyPtr = backend.pointerOf(errBuf);
