...
void operator delete(void* ptr) noexcept {
    ...
    if (ptr != nullptr) {
        polyplug_host_free(static_cast<uint8_t*>(ptr), 0U, alignof(std::max_align_t));
    }
}
...
void operator delete(void* ptr, std::size_t sz) noexcept {
    if (ptr != nullptr) {
        polyplug_host_free(static_cast<uint8_t*>(ptr), sz, alignof(std::max_align_t));
    }
}
...
void operator delete(void* ptr, std::size_t sz, std::align_val_t al) noexcept {
    if (ptr != nullptr) {
        polyplug_host_free(static_cast<uint8_t*>(ptr), sz, static_cast<std::size_t>(al));
    }
}
