...
#QN|Auto-generated by polyplugc. Placed inside the bundle directory as `bundle_dir/manifest.toml`. Runtime reads this for fast pre-load discovery without loading. Contains resolved dependency information for graph building.
...
#QX|Scans configured directories. Every bundle is a **directory** containing `manifest.toml` and the plugin file(s). Both compiled bundles (native, C#) and script bundles (Python, Lua, JS) use the same directory layout. Configured at runtime init in code, no config file. Flat `.so` files at the scan root are ignored.

**Layer 2 — Manifest reading**
#KY|Reads `bundle_dir/manifest.toml` for each discovered directory. A directory without `manifest.toml` is **skipped entirely** — logged and not loaded. A directory without a manifest is not a valid polyplug bundle. Extracts from valid manifests: name, bundle_id, version, runtime, file, provides, function_count, dependencies.
...
```rust
#BX|runtime.load_bundle("./my_plugin/")?;
#NJ|runtime.load_bundle_with("./my_plugin/", LoadOptions {
...
```
#ZS|New version of plugin file (e.g. .so) detected inside bundle directory (inotify / notify)
#TX|        │
#QJ|        ▼
#MB|Watcher uses RecursiveMode::Recursive to detect changes within bundle subdirectories.
#MB|Reload is triggered for the specific plugin file path.
#MB|        │
#MB|        ▼
#MB|Runtime derives bundle directory via path.parent(). Manifest `path` stores the bundle DIRECTORY.
#MB|        │
#MB|        ▼
#MB|Runtime loads new_B.so (via correct loader), runs init, gets new PluginVTable*
        │
        ▼
