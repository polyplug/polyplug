...
| 3 | ContractHandle has only index field (no generation counter) | VERIFIED | `grep -c "generation" crates/polyplug_abi/src/plugin/plugin_handle.rs` returns 0; PluginHandle struct has only `pub index: u32` |
| 4 | find_contract returns ContractHandle without generation validation | VERIFIED | PluginHandle struct has single `index: u32` field; pack() returns `index as u64` (no generation packing) |
...
| `crates/polyplug_abi/src/plugin/plugin_handle.rs` | PluginHandle with only index field | VERIFIED | `pub struct PluginHandle { pub index: u32 }` |
...
| plugin_handle.rs | PluginHandle::pack() | method | WIRED | Returns `index as u64` (no generation) |
| plugin_handle.rs | PluginHandle::null() | method | WIRED | Returns `index: u32::MAX` |
...
| generation removed from PluginHandle | `grep -c "generation" crates/polyplug_abi/src/plugin/plugin_handle.rs` | 0 | PASS |
...
| PluginHandle size | `size_of::<PluginHandle>()` | 4 bytes | PASS |
...
| REG-03 | 02-03-PLAN.md + grep | Remove generation counter from handles (ContractHandle) | SATISFIED | PluginHandle has only `pub index: u32`; `grep -c "generation"` returns 0 |
...
| REG-06 | 02-03-PLAN.md + grep | Update find_contract to return ContractHandle without generation | SATISFIED | PluginHandle has single index field; pack() returns `index as u64` only |
...
- **Generation counter removed:** PluginHandle is 4 bytes (single u32 index)
