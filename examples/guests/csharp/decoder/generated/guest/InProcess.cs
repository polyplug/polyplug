...
public static class InProcessBundleFactory {
    public static InProcessBundle CreateInProcessBundle(
        Func<IntPtr, IPipelineDecoderGuestContract> decoder_pipeline_decoderFactory
    ) {
        var resident = new Resident(
            decoder_pipeline_decoderFactory
        );
    ...
    private sealed class Resident : IDisposable {
        ...
        internal Resident(
            ...
            _interfaces = [
                DecoderInterfaces.CreateInProcessInterface(decoder_pipeline_decoderFactory),
            ];
            _strings = [
                System.Text.Encoding.UTF8.GetBytes("decoder"),
                System.Text.Encoding.UTF8.GetBytes("pipeline.Decoder@1"),
            ];
        ...
        public void Dispose() {
            if (_disposed) return;
            _disposed = true;
            DecoderInterfaces.ReleaseInProcessInterface(_interfaces[0]);
