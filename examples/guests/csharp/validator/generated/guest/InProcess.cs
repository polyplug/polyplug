...
public static class InProcessBundleFactory {
    public static InProcessBundle CreateInProcessBundle(
        Func<IntPtr, IPipelineValidatorGuestContract> validator_pipeline_validatorFactory
    ) {
        var resident = new Resident(
            validator_pipeline_validatorFactory
        );
    ...
    private sealed class Resident : IDisposable {
        ...
        internal Resident(
            ...
            _interfaces = [
                ValidatorInterfaces.CreateInProcessInterface(validator_pipeline_validatorFactory),
            ];
            _strings = [
                System.Text.Encoding.UTF8.GetBytes("validator"),
                System.Text.Encoding.UTF8.GetBytes("pipeline.Validator@1"),
            ];
        ...
        public void Dispose() {
            if (_disposed) return;
            _disposed = true;
            ValidatorInterfaces.ReleaseInProcessInterface(_interfaces[0]);
