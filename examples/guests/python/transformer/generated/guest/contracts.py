...
TRANSFORMER_PLUGIN_NAME_C: ctypes.c_void_p = ctypes.cast(ctypes.c_char_p(TRANSFORMER_PLUGIN_NAME_BYTES), ctypes.c_void_p)
TRANSFORMER_CONTRACT_NAME_C: ctypes.c_void_p = ctypes.cast(ctypes.c_char_p(TRANSFORMER_CONTRACT_NAME_BYTES), ctypes.c_void_p)
...
def transformer_transform_abi(args_ptr: int, out_ptr: int) -> _AbiError:
...
TRANSFORMER_transformer_transform_abi_CFUNC = _DISPATCH_FN_CTYPE(_wrap_sret(transformer_transform_abi))
...
def polyplug_init(host_ptr: int, ctx_ptr: int) -> None:
    ...
    err_TRANSFORMER: AbiError = host.contents.register_guest_contract(
