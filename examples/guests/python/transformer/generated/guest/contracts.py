...
class TRANSFORMERDataTransformerGuestContract:
    def transform(self, input: StringView) -> StringView:
        raise NotImplementedError

_transformer_IMPL: TRANSFORMERDataTransformerGuestContract | None = None
def set_transformer_impl(impl: TRANSFORMERDataTransformerGuestContract) -> None:
...
def transformer_transform_abi(instance: _GuestContractInstance, args_ptr: ctypes.c_void_p, out_ptr: ctypes.c_void_p) -> _AbiError:
    ...
    impl: TRANSFORMERDataTransformerGuestContract | None = _transformer_IMPL
    if impl is None:
...
def TRANSFORMER_create_instance_stub(host: ctypes.c_void_p, args: ctypes.c_void_p) -> _GuestContractInstance:
...
def TRANSFORMER_destroy_instance_stub(host: ctypes.c_void_p, instance: _GuestContractInstance) -> None:
    # Default stub is no-op - users override for cleanup before hot-reload.
    pass

TRANSFORMER_CREATE_INSTANCE_CFUNC = _CREATE_INSTANCE_FN_CTYPE(TRANSFORMER_create_instance_stub)
TRANSFORMER_DESTROY_INSTANCE_CFUNC = _DESTROY_INSTANCE_FN_CTYPE(TRANSFORMER_destroy_instance_stub)

TRANSFORMER_INTERFACE: GuestContractInterface = GuestContractInterface(
    contract_id=0x4775991362CD68EE,
    contract_version=0,
    dispatch_type=DispatchType.VirtualMachine,
    create_instance=ctypes.cast(TRANSFORMER_CREATE_INSTANCE_CFUNC, ctypes.c_void_p),
    destroy_instance=ctypes.cast(TRANSFORMER_DESTROY_INSTANCE_CFUNC, ctypes.c_void_p),
...
def polyplug_init(host_ptr: int, ctx_ptr: int) -> None:
    ...
    err_TRANSFORMER: AbiError = host.contents.register_contract(
        host_ptr, ctypes.byref(TRANSFORMER_DESCRIPTOR), ctypes.byref(TRANSFORMER_INTERFACE)
    )
