"""
examples/guests/python/validator/validator.py
Python guest plugin implementing pipeline.Validator@1.

Contract: validate(data: StringView) -> StringView
Input:  "DECODED:name|value|42"
Output: "VALID:ok" if correct format, "INVALID:<reason>" otherwise
...
VALIDATOR_CONTRACT_ID: int = 0xA553FAB5D11C7AF0

_PLUGIN_NAME_BYTES: bytes = b"validator-python"
_CONTRACT_NAME_BYTES: bytes = b"pipeline.Validator"

_last_result_bytes: list = []


def _py_validate(args_ptr: ctypes.c_void_p, out_ptr: ctypes.c_void_p) -> AbiError:
    """Implements: validate(data: StringView) -> StringView"""
    ...
    result_str: str = _validate_decoded_format(data_str)
...
def _validate_decoded_format(data: str) -> str:
    """Validate that data matches 'DECODED:name|value|number' format."""
    if not data.startswith("DECODED:"):
        return "INVALID:missing DECODED: prefix"

    payload: str = data[len("DECODED:") :]
    parts: list[str] = payload.split("|")

    if len(parts) != 3:
        return "INVALID:expected 3 pipe-separated fields"

    name: str = parts[0]
    value: str = parts[1]
    number_str: str = parts[2]

    if not name:
        return "INVALID:empty name field"

    if not value:
        return "INVALID:empty value field"

    try:
        _parsed: int = int(number_str)
    except ValueError:
        return "INVALID:third field is not an integer"
...
_FN_VALIDATE = _DISPATCH_FN_TYPE(_wrap_sret(_py_validate))

_FUNCTIONS_ARRAY = (ctypes.c_void_p * 1)(
    ctypes.cast(_FN_VALIDATE, ctypes.c_void_p),
)
