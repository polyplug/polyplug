...
extern "C" AbiError polyplug_init(const HostInterface* host, const BundleInitContext* ctx) {
    ...
    AbiError err_REPORTER = host->register_contract(host, &desc_REPORTER, &polyplug_plugin::REPORTER_INTERFACE);
    if (err_REPORTER.code != AbiErrorCode::Ok) return err_REPORTER;
