...
// the instance (deleted in ENCODER_destroy_instance).
...
struct EncoderInstanceState {
    ...
    PipelineEncoderGuestContract* impl;
};
...
// null, or it throws.
static void ENCODER_create_instance(void* adapter_context, VmLoaderData loader_data, const HostApi* host, const void* args, GuestContractInstance* out_instance) noexcept {
    ...
    try {
        PipelineEncoderGuestContract* impl = polyplug_create_encoder(host);
        if (impl == nullptr) {
            *out_instance = GuestContractInstance{nullptr, 0U};
            return;
        }
...
// Destroy an instance created by ENCODER_create_instance: deletes the
// implementation (ownership transferred from the factory) and the payload.
static void ENCODER_destroy_instance(void* adapter_context, VmLoaderData loader_data, const HostApi* host, GuestContractInstance instance) noexcept {
    (void)adapter_context;
    (void)loader_data;  // Native-dispatch contracts ignore the VM loader handle.
...
inline void encoder_encode_abi(void* adapter_context, GuestContractInstance instance, const void* args, void* out, AbiError* out_err) noexcept {
    (void)adapter_context;
    if (instance.data == nullptr) {
    ...
    const auto* state = static_cast<const EncoderInstanceState*>(instance.data);
    try {
        if (args == nullptr) {
...
auto result = state->impl->encode(*static_cast<const StringView*>(args));
