...
//! rust_reporter — Rust native plugin implementing data.Reporter v1.
//!
...
use guest::contracts::DataReporterPlugin;
use guest::vtables::set_rust_reporter_impl;
...
impl DataReporterPlugin for ReporterPlugin {
    fn report(&self, data: StringView) -> Result<StringView, PluginError> {
        ...
        let pipe_separated: String = data_str.replace(',', "|");
        let result: String = format!("REPORTED:{}", pipe_separated);
...
fn init_plugin() {
    let _ = INIT.get_or_init(|| {
        let _ = set_rust_reporter_impl(Box::new(ReporterPlugin));
    });
}
