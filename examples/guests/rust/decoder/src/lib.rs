//! rust_decoder — Rust native plugin implementing pipeline.Decoder v1.0.
//!
...
// ─── Contract ID ──────────────────────────────────────────────────────────────
// FNV1a-64 of "pipeline.Decoder@1" = 0x12F3C106B0C3DC1E

const DECODER_CONTRACT_ID: u64 = 0x12F3C106B0C3DC1E;
...
extern "C" fn plugin_decode(args: *const (), out: *mut ()) -> AbiError {
    ...
    unsafe { decode_impl(args as *const StringView, out as *mut StringView) }
}
...
unsafe fn decode_impl(input: *const StringView, out: *mut StringView) -> AbiError {
...
static DECODER_FNS: [FnPtr; 1] = [FnPtr(plugin_decode as *const ())];

static DECODER_VTABLE: PluginVTable = PluginVTable {
    contract_id: DECODER_CONTRACT_ID,
    contract_version: 0u32,
    function_count: 1,
    functions: DECODER_FNS.as_ptr(),
};

static DECODER_DESCRIPTOR: PluginDescriptor = PluginDescriptor {
    name: StringView {
        ptr: b"rust_decoder".as_ptr(),
        len: 12,
    },
...
pub unsafe extern "C" fn polyplug_init(
    ...
    // SAFETY: emit_trace reads a stable OnceLock — no unsoundness.
    unsafe { emit_trace("[rust_decoder] init") };
    ...
    unsafe {
        (reg.register_plugin)(
            registrar,
            &DECODER_DESCRIPTOR as *const PluginDescriptor,
