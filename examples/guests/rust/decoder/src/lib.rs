...
// ─── Contract ID ──────────────────────────────────────────────────────────────
// FNV1a-64 of "data.Transformer@1" = 0x3D53C682F3F5A9EF

const TRANSFORMER_CONTRACT_ID: u64 = 0x3D53C682F3F5A9EF;
...
extern "C" fn plugin_transform(args: *const (), out: *mut ()) -> AbiError {
    ...
    unsafe { transform_impl(args as *const StringView, out as *mut StringView) }
}
...
unsafe fn transform_impl(input: *const StringView, out: *mut StringView) -> AbiError {
    ...
    let result: String = format!("rust:transform({})", input_str);
    ...
    unsafe { emit_trace("[rust_transformer] transform called") };
...
static TRANSFORMER_DESCRIPTOR: PluginDescriptor = PluginDescriptor {
    name: StringView {
        ptr: b"rust_transformer".as_ptr(),
        len: 16,
    },
    contract_name: StringView {
        ptr: b"data.Transformer".as_ptr(),
        len: 16,
    },
    version_major: 1,
    version_minor: 0,
...
pub unsafe extern "C" fn polyplug_init(registrar: *mut PluginRegistrar) -> AbiError {
    ...
    // SAFETY: emit_trace reads a stable OnceLock — no unsoundness.
    unsafe { emit_trace("[rust_transformer] init") };
