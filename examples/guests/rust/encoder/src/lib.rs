//! rust_encoder — Rust native plugin implementing pipeline.Encoder v1.0.
//!
//! Contract: encode(data: StringView) -> StringView
//! Input:  "TRANSFORMED:NAME|value (transformed)|43"
//! Output: "NAME,value (transformed),43"

// ABI types mirrored from polyplug — frozen per §7 ABI Stability.
// Cannot depend on polyplug crate (cdylib circular dependency).
...
// FNV1a-64 of "pipeline.Encoder@1" = 0x127D1703C6EFB432
const ENCODER_CONTRACT_ID: u64 = 0x127D1703C6EFB432;
...
extern "C" fn plugin_encode(args: *const (), out: *mut ()) -> AbiError {
    ...
    unsafe { encode_impl(args as *const StringView, out as *mut StringView) }
}
...
unsafe fn encode_impl(input: *const StringView, out: *mut StringView) -> AbiError {
    ...
    // Strip "TRANSFORMED:" prefix, then split on "|" and rejoin with ","
    let payload: &str = match input_str.strip_prefix("TRANSFORMED:") {
        Some(p) => p,
        None => {
            return AbiError {
                code: 2,
                message: StringView {
                    ptr: b"missing TRANSFORMED: prefix".as_ptr(),
                    len: 27,
                },
            };
        }
    ...
    let result: String = payload.replace('|', ",");
...
static ENCODER_FNS: [FnPtr; 1] = [FnPtr(plugin_encode as *const ())];

static ENCODER_VTABLE: PluginVTable = PluginVTable {
    contract_id: ENCODER_CONTRACT_ID,
    contract_version: 0u32,
    function_count: 1,
    functions: ENCODER_FNS.as_ptr(),
};

static ENCODER_DESCRIPTOR: PluginDescriptor = PluginDescriptor {
    name: StringView {
        ptr: b"rust_encoder".as_ptr(),
        len: 12,
    },
...
pub unsafe extern "C" fn polyplug_init(
    ...
    // SAFETY: emit_trace reads a stable OnceLock — no unsoundness.
    unsafe { emit_trace("[rust_encoder] init") };
    ...
    unsafe {
        (reg.register_plugin)(
            registrar,
            &ENCODER_DESCRIPTOR as *const PluginDescriptor,
