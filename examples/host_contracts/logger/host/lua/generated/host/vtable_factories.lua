...
-- ABI types for host contract vtables
local ABI_OK = 0
...
function M.create_host_logger_vtable(impl)
    ...
    local functions = ffi.new("void*[1]")
    ...
    vtable.header.function_count = 1
...
-- Memory:
-- The returned vtable is cached and lives for the lifetime of the program.
function M.create_host_logger_vtable_vm(bridge_data, dispatch_fn)
    local vtable = ffi.new("HostContractVTable")
    vtable.header.vtable_version = 1
    vtable.header.contract_id = 0xF53EB5F2845853BBULL
    vtable.header.contract_major = 1
