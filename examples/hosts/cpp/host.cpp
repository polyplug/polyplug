...
static std::string last_error_string() {
    size_t const len = polyplug_error_message_len();
    if (len == 0U) {
        return std::string("(no details)");
    }
    std::vector<uint8_t> buf(len);
    size_t const written = polyplug_last_error(buf.data(), buf.size());
...
int main(int argc, char* argv[]) {
    ...
    OpaqueRuntime* const rt = polyplug_runtime_new();
    if (rt == nullptr) {
        std::cerr << "polyplug_runtime_new failed: " << last_error_string() << std::endl;
        return 1;
    }

    try {
        uint32_t const load_rc = polyplug_load_bundle(
            rt,
            reinterpret_cast<const uint8_t*>(bundle_dir.data()),
            bundle_dir.size()
        );
        if (load_rc != 0U) {
            throw std::runtime_error("polyplug_load_bundle failed: " + last_error_string());
        }
        std::cout << "Bundle loaded." << std::endl;

        uint64_t const packed_handle =
            polyplug_rt_find_by_contract(rt, DECODER_CONTRACT_ID, 0U);
        ...
        OpaqueGuard* const guard = polyplug_rt_resolve_plugin(rt, packed_handle);
        if (guard == nullptr) {
            throw std::runtime_error(
                "polyplug_rt_resolve_plugin failed: " + last_error_string()
            );
        }

        const void* const vt_raw = polyplug_get_vtable(guard);
        if (vt_raw == nullptr) {
            polyplug_guard_free(guard);
            throw std::runtime_error("polyplug_get_vtable returned null");
        }
