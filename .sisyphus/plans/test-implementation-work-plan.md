...
**Execution Strategy**: 6 waves of parallel work, organized by crate and criticality. High-priority safety tests in Wave 1, CLI/parser in Wave 2, concurrency stress in Wave 3, language bindings in Wave 4, integration polish in Wave 5, final verification in Wave 6.
...
### Evidence Capture
- Screenshots of test output for stress tests
- Timing data for performance regression detection
- Coverage reports showing line coverage increase
...
- [ ] **Task 1: Trust Boundary Transition Tests**
  ...
  **Recommended Agent Profile**:
  - **Category**: `deep`
  - **Skills**: `rust-expert`, `ffi-testing`
  ...
  **References**:
  - `src/loader/mod.rs:47-56` - BundleInitGuard RAII guard
  - `src/loader/mod.rs:200-230` - TLS variable initialization
  - `src/runtime.rs:150-180` - Phase transition logic
  - `TRUST_MODEL.md` - Trust boundary documentation

  **Acceptance Criteria**:
  - [ ] Test: bundle_id=0 cannot escape enforcement (returns error)
  ...
  **QA Scenarios**:
  ...
  Scenario: bundle_id=0 bypass attempt
    ...
    Expected: PolyplugError::AccessDenied or similar
...
- [ ] **Task 2: Quiescence Race Window Tests**
  ...
  **Must NOT do**: Use sleep-based timing (flaky)

  **Recommended Agent Profile**:
  - **Category**: `ultrabrain`
  - **Skills**: `rust-expert`, `concurrency-testing`
  ...
  **References**:
  - `src/reload.rs:215-228` - Quiescence loop
  - `src/reload.rs:180-200` - Old Arc drop

  **Acceptance Criteria**:
  - [ ] Test: Guard acquired, reload started, new resolve attempted during quiescence
  - [ ] Test: Verify no use-after-free during window
  - [ ] Test: Verify old vtable inaccessible after drop
  - [ ] Test: Stress test with rapid reload cycles (1000x)
  ...
  **QA Scenarios**:
  ...
  Scenario: Quiescence race
    ...
    Steps:
      ...
      2. Trigger reload
      3. Attempt to resolve new guard during quiescence
    Expected: New guard gets new vtable, old Arc not dropped yet
...
- [ ] **Task 3: Registrar Callback Security Tests**
  ...
  **What to do**: Test registrar_callback resilience against malicious inputs

  **Must NOT do**: Cause actual undefined behavior in test (use mock data)

  **Recommended Agent Profile**:
  - **Category**: `deep`
  - **Skills**: `rust-expert`, `security-testing`
  ...
  **References**:
  - `src/loader/mod.rs:426-461` - registrar_callback
  - `src/loader/mod.rs:400-425` - TLS registry pointer

  **Acceptance Criteria**:
  - [ ] Test: Null registry TLS pointer returns error code 1
  - [ ] Test: Vtable with function_count > actual array length detected
  - [ ] Test: Malformed descriptor with garbage pointer handled gracefully
  ...
  **QA Scenarios**:
  ...
  Scenario: Null registry pointer
    ...
    Expected: Error code 1 returned
...
- [ ] **Task 4: pack_handle/unpack_handle Unit Tests**
  ...
  **Must NOT do**: Test private functions directly (test through public FFI)
  ...
  **References**:
  - `src/ffi.rs:19-36` - pack_handle/unpack_handle
...
- [ ] **Task 5: CLI Argument Validation Tests**
  ...
  **Must NOT do**: Test actual file system operations (use mock filesystem or tempdir)

  **Recommended Agent Profile**:
  - **Category**: `quick`
  - **Skills**: `cli-testing`, `rust-expert`
...
- [ ] **Task 6: Parser Error Handling Tests**
  ...
  **Recommended Agent Profile**:
  ...
  - **Skills**: `rust-expert`, `parser-testing`
  ...
  **Acceptance Criteria**:
  - [ ] Test: Malformed TOML syntax errors (line-specific messages)
...
- [ ] **Task 7: TOML Malformed Input Tests**
  ...
  **QA Scenarios**:
  ...
  Scenario: Malformed TOML
    ...
    Expected: TOML parse error with line number
...
- [ ] **Task 8: Concurrent Registry Stress Tests**
  ...
  **References**:
  - `src/registry.rs` - Registry implementation
  - `src/abi.rs` - resolve_plugin
...
- [ ] **Task 9: FFI Robustness Tests**
  ...
  **Recommended Agent Profile**:
  - **Category**: `deep`
  - **Skills**: `rust-expert`, `ffi-testing`, `security-testing`
  ...
  **References**:
  - `src/ffi.rs` - FFI entry points
  - `src/abi.rs` - ABI types

  **Acceptance Criteria**:
  - [ ] Test: NULL StringView with non-zero length
  - [ ] Test: Invalid UTF-8 in StringView
  - [ ] Test: Misaligned Buffer pointer
  - [ ] Test: StringView with embedded NULLs
  - [ ] Test: Cross-thread StringView/Buffer usage
  - [ ] Test: Buffer cap smaller than len

  **QA Scenarios**:
  ```
  Scenario: NULL StringView
    Tool: cargo test --test integration_ffi_robustness
    Steps:
      1. Create plugin that passes StringView { ptr: null, len: 5 }
      2. Call host function
    Expected: ABI_ERROR_INVALID_ARGS, no panic
    Evidence: .sisyphus/evidence/task-9-ffi-robustness.txt
  ```

  **Commit**: YES
  - Message: `test: add FFI robustness tests`
  - Files: `crates/polyplug/tests/integration_ffi_robustness.rs`

- [ ] **Task 10: LAST_ERROR Thread Isolation Tests**

  **What to do**: Test thread-local error handling
  ...
  **References**:
  - `src/ffi.rs:11-17` - LAST_ERROR thread-local
  - `src/ffi.rs:250-270` - polyplug_last_error
  ...
  **QA Scenarios**:
  ...
  Scenario: Thread isolation
    ...
    Steps:
      1. Thread A sets error
      2. Thread B reads error
...
- [ ] **Task 11: .NET Loader Tests**
  ...
  **Recommended Agent Profile**:
  - **Category**: `unspecified-high`
  - **Skills**: `dotnet-expert`, `rust-expert`
...
- [ ] **Task 13: Deno Loader Tests**
  ...
  **Recommended Agent Profile**:
  - **Category**: `unspecified-high`
  - **Skills**: `js-expert`, `rust-expert`, `deno-expert`
...
- [ ] **Task 15: Python Loader Tests**
  ...
  **Recommended Agent Profile**:
  - **Category**: `unspecified-high`
  - **Skills**: `python-expert`, `rust-expert`
...
- [ ] **Task 16: Cross-Language Type Mapping Tests**
  ...
  **What to do**: Verify type mappings are consistent across languages

  **Must NOT do**: Require external toolchains (test output strings only)

  **Recommended Agent Profile**:
  - **Category**: `unspecified-high`
  - **Skills**: `codegen-expert`, `multi-language`
  - **Reasoning**: Cross-language ABI compatibility
  ...
  **Acceptance Criteria**:
  - [ ] Test: U8 maps to correct type in all languages
  - [ ] Test: U64 maps to correct type (BigInt in JS)
  - [ ] Test: StringView mapping consistency
  - [ ] Test: Buffer mapping consistency
  - [ ] Test: Struct layout comments match
  - [ ] Test: Enum repr type mapping

  **QA Scenarios**:
  ```
  Scenario: Type matrix
    Tool: cargo test --test type_mapping_matrix
    Steps:
      1. Generate code for all primitives in all languages
      2. Verify expected type strings present
    Expected: All mappings correct
    Evidence: .sisyphus/evidence/task-16-type-mapping.txt
  ```

  **Commit**: YES
  - Message: `test: add cross-language type mapping tests`
  - Files: `crates/polyplug_codegen/tests/type_mapping_matrix.rs`

- [ ] **Task 17: Generator Output Correctness Tests**

  **What to do**: Verify generated code is syntactically correct

  **Must NOT do**: Require compilation (structure only)

  **Recommended Agent Profile**:
  - **Category**: `unspecified-high`
  - **Skills**: `codegen-expert`
  ...
  **Acceptance Criteria**:
  - [ ] Test: VTable function order matches contract
  - [ ] Test: Slot indices are correct
  - [ ] Test: Header guards present (C++, C#)
  - [ ] Test: Namespace generation correct
  - [ ] Test: Function signatures match contract

  **QA Scenarios**:
  ```
  Scenario: VTable order
    Tool: cargo test --test generator_output
    Steps:
      1. Generate vtable for contract with multiple functions
      2. Verify order matches definition
    Expected: Order preserved
    Evidence: .sisyphus/evidence/task-17-generator-output.txt
  ```

  **Commit**: YES
  - Message: `test: add generator output correctness tests`
  - Files: `crates/polyplug_codegen/tests/generator_output.rs`
...
- [ ] **Task 19: Version Parsing Unit Tests**
  ...
  **What to do**: Unit tests for version.rs

  **Must NOT do**: Test comparison (covered), test parsing edge cases
  ...
  **Acceptance Criteria**:
  - [ ] Test: "1" parses correctly
  - [ ] Test: "1.2" parses correctly
  - [ ] Test: "1.2.3" parses correctly
  - [ ] Test: "1.2.3.4" overflow handling
  - [ ] Test: Non-numeric components error
  - [ ] Test: Negative numbers error
  - [ ] Test: Pre-release versions ("1.0.0-alpha")
  - [ ] Test: Wildcard requirements ("^1.2.0", "~1.2.0")

  **QA Scenarios**:
  ```
  Scenario: Version parse
    Tool: cargo test --lib version::tests::parse_edge_cases
    Steps:
      1. Parse various version strings
    Expected: Correct parsing or errors
    Evidence: .sisyphus/evidence/task-19-version-parsing.txt
  ```

  **Commit**: YES
  - Message: `test: add version parsing unit tests`
...
- [ ] **Task 20: Error Formatting Unit Tests**
  ...
  **Must NOT do**: Test error variants, test message formatting
  ...
  **Acceptance Criteria**:
  - [ ] Test: PolyplugError Display for all variants
  - [ ] Test: LoaderError Display for all variants
  - [ ] Test: RegistryError Display for all variants
  - [ ] Test: GraphError Display for all variants
  - [ ] Test: PolyplugcError Display for all variants
  - [ ] Test: Error messages contain expected context (IDs, names)
  - [ ] Test: Special characters in error messages handled
...
- [ ] **Task 21: Hash Function Stability Tests**
  ...
  **Must NOT do**: Test hash algorithm (covered), test contract_id/bundle_id
  ...
  **Acceptance Criteria**:
  ...
  - [ ] Test: Hash collision resistance (theoretical check)
...
- [ ] **Task 22: Loader/Manifest Unit Tests**
  ...
  **Must NOT do**: Test file I/O (use mock data)
  ...
  **Acceptance Criteria**:
  ...
  - [ ] Test: Scanner directory traversal limits
  - [ ] Test: Scanner symlink following behavior
...
- [ ] **Task 23: Graph Edge Case Unit Tests**
  ...
  **Acceptance Criteria**:
  ...
  - [ ] Test: Multi-bundle cycle error message
  - [ ] Test: Version-aware graph resolution
  - [ ] Test: Disconnected graph handling
  - [ ] Test: Deep chain handling (20 bundles, not 1000)
...
- [ ] **Task F1: Plan Compliance Audit**
  ...
  **Acceptance Criteria**:
  ...
  - [ ] Verify: FFI robustness tests exist and pass
  - [ ] Verify: CLI/parser tests exist and pass
  - [ ] Verify: No duplicate test coverage (8 items removed)
...
- [ ] **Task F2: Test Isolation Documentation**
  ...
  **Acceptance Criteria**:
  - [ ] Document: OnceLock global state behavior
  - [ ] Document: Test isolation requirements (separate binaries)
  - [ ] Document: Runtime::builder() limitation
  - [ ] Document: How to write isolated tests
